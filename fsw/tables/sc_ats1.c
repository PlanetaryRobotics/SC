...
**         The ATP relies on a sentinel word of zeroes at the end of an ATS
*table
...
** Added note describing need for sentinal word of zeros at the end of an ATS
*table.  Updated table to include sentinal word of zeros.
...
** Remove pad from ATS and RTS structures, change 32 bit ATS time to two 16 bit
*values
** Revision 1.1 2010/03/16 15:43:07EDT lwalling
** Initial revision
** Member added to project
*c:/MKSDATA/MKS-REPOSITORY/CFS-REPOSITORY/sc/fsw/tables/project.pj
**
*************************************************************************/
...
/*
** Length of cmd pkt data (in bytes minus one) that follows primary header
*(thus, 0xFFFF = 64k)
*/
...
static CFE_TBL_FileDef_t CFE_TBL_FileDef __attribute__((__used__)) = {
    "ATS_Table1", "SC.ATS_TBL1", "SC Sample ATS_TBL1", "sc_ats1.tbl",
    (SC_ATS_BUFF_SIZE * sizeof(uint16))};

/*
** Sample ATS_TBL1 Table Data
*/
uint16 ATS_Table1[SC_ATS_BUFF_SIZE] = {
    /* cmd num, <---- cmd exe time ---->   <---------------------------- cmd pkt
       primary header ---------------------------->  <----- cmd pkt 2nd header
       ---->   <-- opt data ---> */
    1,
    CMD1_TIME_A,
    CMD1_TIME_B,
    ...
    ((SC_RESET_COUNTERS_CC << 8) | CMD4_XSUM),
    0,
    0,
    0,
    0,
