...
**   Update SB msg size comments, change CDS cfg def from ifdef or ifndef to if
*true or if false
...
**
**  \par Limits:
**       This parameter can't be larger than 999.This parameter will dicate the
*size of
**       The RTS Info Table.
*/
...
/** \sccfg The last RTS that will be sent with an #SC_RTS_START_INF_EID event
 *message
 **
 **  \par Description:
 **       When all RTS's are started, the #SC_RTS_START_INF_EID event message is
 *sent out.
 ...
 **
 **  \par Description:
 **       The prefix of the ATS Command Status table names. Note that actual
 *table names
...
**
**  \par Description:
**       This parameter defines what type of time SC should use for sending uot
*its commands
**
**  \par Limits:
