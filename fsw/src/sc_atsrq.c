...
**   Compiling under GCC 4.4.7 does not cause unitialized warnings for these
variables.
...
**   Cleanup function return code definitions, create error specific event
numbers
**   Revision 1.7 2010/04/21 15:38:24EDT lwalling
**   Moved prototype for SC_BeginAts to header file, added cmd handler for
Append ATS
...
void SC_StartAtsCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_StartAtsCmd_t))) {
            ...
            if (SC_OperData.AtsCtrlBlckAddr->AtpState == SC_IDLE) {
                ...
                if (SC_OperData.AtsInfoTblAddr[AtsIndex].NumberOfCommands > 0) {
                    ...
                    if (SC_BeginAts(AtsIndex, 0)) {
                        ...
                        CFE_EVS_SendEvent(
                            SC_STARTATS_CMD_INF_EID, CFE_EVS_INFORMATION,
                            "ATS %c Execution Started", (AtsIndex ? 'B' : 'A'));
                    } else { /* could not start the ats, all commands were
                                skipped */
                ...
                } else { /* the ats didn't have any commands in it */
        ...
        } else { /* the specified ATS id is not valid */

            CFE_EVS_SendEvent(SC_STARTATS_CMD_INVLD_ID_ERR_EID, CFE_EVS_ERROR,
...
void SC_StopAtsCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_NoArgsCmd_t))) {
        ...
        if (Result == CFE_SUCCESS) {
            CFE_EVS_SendEvent(SC_STOPATS_CMD_INF_EID, CFE_EVS_INFORMATION,
                              "ATS %c stopped", TempAtsChar);
        } else {
            CFE_EVS_SendEvent(SC_STOPATS_NO_ATS_INF_EID, CFE_EVS_INFORMATION,
...
boolean SC_BeginAts(uint16 AtsIndex, uint16 TimeOffset) {
    ...
    if (TimeIndex == SC_OperData.AtsInfoTblAddr[AtsIndex].NumberOfCommands) {
        CFE_EVS_SendEvent(SC_ATS_SKP_ALL_ERR_EID, CFE_EVS_ERROR,
    ...
    } else { /* there is at least one command to execute */
        ...
        CFE_EVS_SendEvent(SC_ATS_ERR_SKP_DBG_EID, CFE_EVS_DEBUG,
                          "ATS started, skipped %d commands", CmdsSkipped);
        /*
...
void SC_KillAts(void) {
    if (SC_OperData.AtsCtrlBlckAddr->AtpState != SC_IDLE) {
        ...
        SC_OperData.AtsInfoTblAddr[SC_OperData.AtsCtrlBlckAddr->AtsNumber - 1]
            .AtsUseCtr++;
    }
    /*
...
void SC_GroundSwitchCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_NoArgsCmd_t))) {
        ...
        if (SC_OperData.AtsCtrlBlckAddr->AtpState == SC_EXECUTING) {
            ...
            if (SC_OperData.AtsInfoTblAddr[NewAtsNum].NumberOfCommands > 0) {
                ...
                CFE_EVS_SendEvent(SC_SWITCH_ATS_CMD_INF_EID,
                                  CFE_EVS_INFORMATION, "Switch ATS is Pending");
            } else { /* the other ATS does not have any commands in it */

                CFE_EVS_SendEvent(
                    SC_SWITCH_ATS_CMD_NOT_LDED_ERR_EID, CFE_EVS_ERROR,
        ...
        } else { /* the ATP is not currently executing any commands */

            CFE_EVS_SendEvent(SC_SWITCH_ATS_CMD_IDLE_ERR_EID, CFE_EVS_ERROR,
        ...
        if (SC_OperData.AtsCtrlBlckAddr->AtpState == SC_EXECUTING) {
            ...
            if (SC_OperData.AtsInfoTblAddr[NewAtsNum].NumberOfCommands > 0) {
                ...
                if (SC_BeginAts(NewAtsNum, 1)) {
                    ...
                    CFE_EVS_SendEvent(
                        SC_ATS_SERVICE_SWTCH_INF_EID, CFE_EVS_INFORMATION,
                        "ATS Switched from %c to %c", (OldAtsNum ? 'B' : 'A'),
                        (NewAtsNum ? 'B' : 'A'));
            ...
            CFE_EVS_SendEvent(SC_ATS_SERVICE_SWITCH_IDLE_ERR_EID, CFE_EVS_ERROR,
                              "Switch ATS Rejected: ATP is idle");
        } /* end if */
...
boolean SC_InlineSwitch(void) {
    ...
    if (SC_OperData.AtsInfoTblAddr[NewAtsNum].NumberOfCommands > 0) {
        ...
        if (SC_BeginAts(NewAtsNum, 0)) {
            ...
            CFE_EVS_SendEvent(SC_ATS_INLINE_SWTCH_INF_EID, CFE_EVS_INFORMATION,
            ...
            SC_AppData.CmdCtr++;
            ReturnCode = TRUE;
        } else { /* all of the commands in the new ats were skipped */
        ...
        CFE_EVS_SendEvent(SC_ATS_INLINE_SWTCH_NOT_LDED_ERR_EID, CFE_EVS_ERROR,
                          "Switch ATS Failure: Destination ATS Not Loaded");
        /*
        ...
        if (SC_OperData.AtsCtrlBlckAddr->AtpState == SC_EXECUTING) {
            ...
            while (TimeIndex <
                ...
                if (SC_CompareAbsTime(JumpTime, ListCmdTime)) {
                    ...
                    **  if the command has any other status, SC_SKIPPED,
                    *SC_EXECUTED,
                    **   etc, then leave the status alone.
                    */
                    if (SC_OperData.AtsCmdStatusTblAddr[AtsIndex][CmdIndex] ==
                        SC_LOADED) {
            ...
            if (TimeIndex ==
                ...
                CFE_EVS_SendEvent(
                    SC_JUMP_ATS_INF_EID, CFE_EVS_INFORMATION,
                    "Next ATS command time in the ATP was set to %s",
                    TimeBuffer);
                if (NumSkipped > 0) {
                    /* We skipped come commands, but not all of them */
                    CFE_EVS_SendEvent(
                        SC_JUMP_ATS_SKIPPED_DBG_EID, CFE_EVS_DEBUG,
                        "Jump Cmd: Skipped %d ATS commands", NumSkipped);
                }

            }    /* end if */
        } else { /*  There is not a running ATS */

            CFE_EVS_SendEvent(SC_JUMPATS_CMD_NOT_ACT_ERR_EID, CFE_EVS_ERROR,
...
void SC_ContinueAtsOnFailureCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket,
                           sizeof(SC_SetContinueAtsOnFailureCmd_t))) {
        ...
        if (State != TRUE && State != FALSE) {
            ...
            CFE_EVS_SendEvent(SC_CONT_CMD_DEB_EID, CFE_EVS_DEBUG,
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_AppendAtsCmd_t))) {
        ...
        if (AtsIndex >= SC_NUMBER_OF_ATS) {
            ...
            CFE_EVS_SendEvent(SC_APPEND_CMD_SRC_ERR_EID, CFE_EVS_ERROR,
                              ...
                              'A' + AtsIndex);
        } else if ((SC_OperData.AtsInfoTblAddr[AtsIndex].AtsSize +
            ...
            CFE_EVS_SendEvent(SC_APPEND_CMD_FIT_ERR_EID, CFE_EVS_ERROR,
                              "Append ATS %c error: ATS size = %d, Append size "
                              "= %d, ATS buffer = %d",
                              'A' + AtsIndex,
                              (int)SC_OperData.AtsInfoTblAddr[AtsIndex].AtsSize,
                              SC_AppData.AppendWordCount, SC_ATS_BUFF_SIZE);
        } else {
            /* store ATS selection from most recent ATS Append command */
