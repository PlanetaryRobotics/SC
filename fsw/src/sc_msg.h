...
**   Change command counter from 16 bits to 8 bits, remove obsolete alignment
*pad word
...
typedef struct {
    ...
    /**< \brief In the event of ATS execution failure (ats command fails
     checksum) , the ATS execution will continue if this flag is set to TRUE and
     will stop if this flag is set to FALSE
     */
    ...
    /**< \brief  The OFFSET in the RTS buffer of the command that had an error
    It will be a WORD value i.e. 1st command had an error, this value would be
    0, if the 2nd command started at int8 10 in the buffer, this value would
    be 5. */
    ...
    uint32
        AtpFreeBytes[SC_NUMBER_OF_ATS]; /**< \sctlmmnemonic \SC_ATPFREEBYTES */
    ...
    uint16 RtsExecutingStatus[(SC_NUMBER_OF_RTS + 15) /
                              16]; /**< \sctlmmnemonic \SC_RTSEXEC */
    /**< \brief RTS executing status bit map where each uint16 represents 16 RTS
     numbers.  Note: array index numbers and bit numbers use base zero indexing,
     but RTS numbers use base one indexing.  Thus, the LSB (bit zero) of uint16
     array index zero represents RTS number 1, and bit one of uint16 array index
     zero represents RTS number 2, etc.  If an RTS is IDLE, then the
     corresponding bit is zero. If an RTS is EXECUTING, then the corresponding
     bit is one. */

    uint16 RtsDisabledStatus[(SC_NUMBER_OF_RTS + 15) /
                             16]; /**< \sctlmmnemonic \SC_RTSDISABLED */
    /**< \brief RTS disabled status bit map where each uint16 represents 16 RTS
     numbers.  Note: array index numbers and bit numbers use base zero indexing,
     but RTS numbers use base one indexing.  Thus, the LSB (bit zero) of uint16
     array index zero represents RTS number 1, and bit one of uint16 array index
     zero represents RTS number 2, etc.  If an RTS is ENABLED, then the
     corresponding bit is zero. If an RTS is DISABLED, then the corresponding
     bit is one. */
...
**  For command details see #SC_NOOP_CC, #SC_RESET_COUNTERS_CC, #SC_STOP_ATS_CC,
*#SC_SWITCH_ATS_CC
...
typedef struct {
    uint8 CmdHeader[CFE_SB_CMD_HDR_SIZE];
    uint16
        AtsId; /**< \brief The ID of the ATS to start, 1 = ATS_A, 2 = ATS_B */
} SC_StartAtsCmd_t;

/**
**  \brief RTS Id Command
**  For command details see #SC_START_RTS_CC, #SC_STOP_RTS_CC,
*#SC_DISABLE_RTS_CC, #SC_ENABLE_RTS_CC
*/
typedef struct {
    uint8 CmdHeader[CFE_SB_CMD_HDR_SIZE];
    uint16 RtsId; /**< \brief The ID of the RTS to start, 1 through
                     #SC_NUMBER_OF_RTS */
} SC_RtsCmd_t;
...
typedef struct {
    uint8 CmdHeader[CFE_SB_CMD_HDR_SIZE];
    uint16 ContinueState; /**< \brief TRUE or FALSE, to continue ATS after a
                             failure  */
...
typedef struct {
    uint8 CmdHeader[CFE_SB_CMD_HDR_SIZE];
    uint16 AtsId; /**< \brief The ID of the ATS to append to, 1 = ATS_A, 2 =
                     ATS_B */
} SC_AppendAtsCmd_t;
...
**  For command details see #SC_START_RTSGRP_CC, #SC_STOP_RTSGRP_CC,
*#SC_DISABLE_RTSGRP_CC, #SC_ENABLE_RTSGRP_CC
*/
typedef struct {
    uint8 CmdHeader[CFE_SB_CMD_HDR_SIZE];
    uint16 FirstRtsId; /**< \brief ID of the first RTS to act on, 1 through
                          #SC_NUMBER_OF_RTS */
    uint16 LastRtsId;  /**< \brief ID of the last RTS to act on, 1 through
                          #SC_NUMBER_OF_RTS */
} SC_RtsGrpCmd_t;
#endif
