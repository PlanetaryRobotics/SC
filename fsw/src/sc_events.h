...
**   SC_Loads : SC_Insert: Adds validation of arguments passed in. Otherwise an
*index out of bounds condition could occur
...
**   Changed event msgs to have 'RTS' or 'ATS' instead of "Real Time Sequence",
*etc.
**   Revision 1.6 2011/09/26 09:41:30GMT-08:00 lwalling
**   Remove CDS specific events, create common events to replace CDS/no CDS
*event pairs
...
**   Remove ATS table verify command checksum error event -
*SC_VERIFY_ATS_SUM_ERR_EID
**   Revision 1.2 2010/12/10 15:20:00EST rperera
**
**   Revision 1.1 2010/12/10 15:18:22EST rperera
**   Initial revision
**   Member added to project
*c:/MKSDATA/MKS-REPOSITORY/CFS-REPOSITORY/sc/fsw/src/project.pj
*************************************************************************/
#ifndef _sc_events_
#define _sc_events_
 ...
 **
 **  \par Cause:
 **  This event message is issued when a command is recieved, but it is not of
 *the expected
 **  length
 **
...
/** \brief <tt> 'RTS Table Registration Failed for RTS \%d, returned: 0x\%08X'
 *</tt>
 **  \event <tt> 'RTS Table Registration Failed for RTS \%d, returned: 0x\%08X'
 *</tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **  This event message is issued when a call to #CFE_TBL_Register for an RTS
 *table failed
 **
 **  The \c RTS field contains the RTS ID of the table that failed and the \c
 *returned field
 **  contains the error code returned from #CFE_TBL_Register
 */
#define SC_REGISTER_RTS_TBL_ERR_EID 10

/** \brief <tt> 'ATS Table Registration Failed for ATS \%d, returned: 0x\%08X'
 *</tt>
 **  \event <tt> 'ATS Table Registration Failed for ATS \%d, returned: 0x\%08X'
 *</tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **  This event message is issued when a call to #CFE_TBL_Register for an ATS
 *table failed
 **
 **  The \c ATS field contains the ATS ID of the table that failed and the \c
 *returned field
 **  contains the error code returned from #CFE_TBL_Register
 */
...
/** \brief <tt> 'RTS control block table register failed, returned: 0x\%08X'
 *</tt>
 **  \event <tt> 'RTS control block table register failed, returned: 0x\%08X'
 *</tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'ATS control block table register failed, returned: 0x\%08X'
 *</tt>
 **  \event <tt> 'ATS control block table register failed, returned: 0x\%08X'
 *</tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'ATS command status table register failed for ATS \%d, returned:
 *0x\%08X' </tt>
 **  \event <tt> 'ATS command status table register failed for ATS \%d,
 *returned: 0x\%08X' </tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **  This event message is issued when one of the ATS command status tables
 *fails table
 **  registration
 **
 **  The \c ATS field contains the ATS Id for the table that failed, and the \c
 *returned
...
**
**  \par Cause:
**  This event message is issued when a #SC_SWITCH_ATS_CC command is issued, but
*the
...
**
**  \par Cause:
**  This event message is issued when an ATS switch is scheduled and has been
*switched
**
**  The \c from field contains old ATS and the \c to field contains the new
*running ATS
*/
...
**
**  \par Cause:
**  This event message is issued when an ATS switch is scheduled, but there are
*no commands
...
**
**  \par Cause:
**  This event message is issued when an ATS switch is scheduled, but there is
*no ATS running
...
**
**  \par Cause:
**  This event message is issued when an ATS is scheduled in-line and the switch
*is successful
**
**  The \c from field contains old ATS and the \c to field contains the new
*running ATS
...
**
**  \par Cause:
**  This event message is issued when a #SC_JUMP_ATS_CC command was issued, and
*the time to
...
**
**  \par Cause:
**  This event message is issued when a #SC_JUMP_ATS_CC command was executed
*successfully
**
**
...
**
**  \par Cause:
**  This event message is issued when a #SC_JUMP_ATS_CC command was received,
*but there
**  is no ATS currently running
*/
#define SC_JUMPATS_CMD_NOT_ACT_ERR_EID 41

/** \brief <tt> 'Continue ATS On Failure command  failed, invalid state: \%d"'
 *</tt>
 **  \event <tt> 'Continue ATS On Failure command  failed, invalid state: \%d"'
 *</tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **  This event message is issued when a #SC_CONTINUE_ATS_ON_FAILURE_CC command
 *was received, but the state
...
**
**  \par Cause:
**  This event message is issued when the #SC_CONTINUE_ATS_ON_FAILURE_CC command
*was recieved and
...
**
**  \par Cause:
**  This event message is issued when a command from an ATS is about to be sent
*out
...
**
**  \par Cause:
**  This event message is issued when a ATS command that was about to be sent
*out
**  failed checksum validation, and the Continue-ATS-on_checksum-Failure flag
*was
**  set to 'FALSE'
**
...
/** \brief <tt> 'ATS Command Distribution Failed, Cmd Number: \%d, SB returned:
 *0x\%08X' </tt>
 **  \event <tt> 'ATS Command Distribution Failed, Cmd Number: \%d, SB returned:
 *0x\%08X' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'ATS Command Number Mismatch: Command Skipped, expected: \%d
 *received: \%d' </tt>
 **  \event <tt> 'ATS Command Number Mismatch: Command Skipped, expected: \%d
 *received: \%d' </tt>
 **
 **  \par Type: ERROR
 **
 ...
 **
 **  The \c expected field is the command number that was in the ATS control
 *block
...
/** \brief <tt> 'RTS \%03d Command Distribution Failed: RTS Stopped. SB returned
 *0x%08X' </tt>
 **  \event <tt> 'RTS \%03d Command Distribution Failed: RTS Stopped. SB
 *returned 0x%08X' </tt>
 **
 **  \par Type: ERROR
 **
...
**
**  \par Cause:
**  This event message is issued when and RTS table is loaded, but there is an
*invalid
...
/** \brief <tt> 'RTS Ctrl Blck table failed Getting Address, returned: 0x\%08X'
 *</tt>
 **  \event <tt> 'RTS Ctrl Blck table failed Getting Address, returned: 0x\%08X'
 *</tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **  This event message is issued when getting the address of the Rts Control
 *Block table
 **  failed
 **
 **  The returned \c field contains the result from #CFE_TBL_GetAddress
...
/** \brief <tt> 'ATS Ctrl Blck table failed Getting Address, returned: 0x\%08X'
 *</tt>
 **  \event <tt> 'ATS Ctrl Blck table failed Getting Address, returned: 0x\%08X'
 *</tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **  This event message is issued when getting the address of the Ats Control
 *Block table
 **  failed
 **
 **  The returned \c field contains the result from #CFE_TBL_GetAddress
...
/** \brief <tt> 'ATS Cmd Status table for ATS \%d failed Getting Address,
 *returned: 0x\%08X' </tt>
 **  \event <tt> 'ATS Cmd Status table for ATS \%d failed Getting Address,
 *returned: 0x\%08X' </tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **  This event message is issued when getting the address of an Ats Command
 *Status table
 **  failed
 **
...
**
**  \par Cause:
**  This event message is issued when a #SC_START_RTS_CC cmd is recieved and is
*sucessful
**
...
/** \brief <tt> 'Start RTS \%03d Rejected: Invld Len Field for 1st Cmd in
 *Sequence. Invld Cmd Length = \%d' </tt>
 **  \event <tt> 'Start RTS \%03d Rejected: Invld Len Field for 1st Cmd in
 *Sequence. Invld Cmd Length = \%d' </tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **  This event message is issued when an RTS is started but the first command
...
/** \brief <tt> 'Start RTS \%03d Rejected: RTS Not Loaded or In Use, Status:
 *\%d' </tt>
 **  \event <tt> 'Start RTS \%03d Rejected: RTS Not Loaded or In Use, Status:
 *\%d' </tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **  This event message is issued when an RTS is tried to be started, but the
 *RTS is not
 **  marked as #SC_LOADED
 **
 **  The \c RTS field contains the RTS ID that was rejected, and the \c Status
 *field
...
**
**  \par Cause:
**  This event message is issued when a #SC_START_RTS_CC command was recieved,
*but the
**  RTS is disabled
**
...
**
**  \par Cause:
**  This event message is issued when an #SC_STOP_RTS_CC command is received and
*exexuted sucessfully
**
...
**
**  \par Cause:
**  This event message is issued when a #SC_STOP_RTS_CC command was rejected
*because the
...
**
**  \par Cause:
**  This event message is issued when a #SC_ENABLE_RTS_CC command was recieved,
*and executed sucessfully
**
...
**
**  \par Cause:
**  This event message is issued when trying to get the next RTS command to
*execute,
...
/** \brief <tt> 'Invalid Length Field in RTS Command, RTS \%03d Aborted. Length:
 *\%d, Max: \%d' </tt>
 **  \event <tt> 'Invalid Length Field in RTS Command, RTS \%03d Aborted.
 *Length:
 *\%d, Max: \%d' </tt>
 **
 **  \par Type: ERROR
...
**
**  \par Cause:
**  This event message is issued when trying to register the Append ATS Info
*Table
**  dump only table fails
**
**  The \c returned field contains the result from #CFE_TBL_Register
...
/** \brief <tt> 'Append ATS Info table failed Getting Address, returned:
 *0x\%08X' </tt>
 **  \event <tt> 'Append ATS Info table failed Getting Address, returned:
 *0x\%08X' </tt>
 **
 **  \par Type: ERROR
 **
 **  \par Cause:
 **  This event message is issued when getting the address of the Append ATS
 *Info table
 **  failed
 **
 **  The returned \c field contains the result from #CFE_TBL_GetAddress
...
/** \brief <tt> 'Append ATS table failed Getting Address, returned: 0x\%08X'
 *</tt>
 **  \event <tt> 'Append ATS table failed Getting Address, returned: 0x\%08X'
 *</tt>
 **
 **  \par Type: ERROR
 **
...
**
**  \par Cause:
**  This event message indicates a failure to register the table with cFE Table
*Services
**
...
/** \brief <tt> 'Update Append ATS Table: load count = \%d, command count = \%d,
 *byte count = \%d' </tt>
 **  \event <tt> 'Update Append ATS Table: load count = \%d, command count =
 *\%d, byte count = \%d' </tt>
 **
 ...
 **
 **  The displayed values include a load counter to identify this particular
 *update,
 **  the number of valid ATS command entries in the table and the size (in
 *bytes) of
...
/** \brief <tt> 'Append ATS \%c error: ATS size = \%d, Append size = \%d, ATS
 *buffer = \%d' </tt>
 **  \event <tt> 'Append ATS \%c error: ATS size = \%d, Append size = \%d, ATS
 *buffer = \%d' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'Verify ATS Table error: invalid command number: buf index =
 *\%d, cmd num = \%d' </tt>
 **  \event <tt> 'Verify ATS Table error: invalid command number: buf index =
 *\%d, cmd num = \%d' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'Verify ATS Table error: buffer full: buf index = \%d, cmd num =
 *\%d, buf words = \%d' </tt>
 **  \event <tt> 'Verify ATS Table error: buffer full: buf index = \%d, cmd num
 *=
 *\%d, buf words = \%d' </tt>
 **
 **  \par Type: ERROR
...
/** \brief <tt> 'Verify ATS Table error: invalid length: buf index = \%d, cmd
 *num = \%d, pkt len = \%d' </tt>
 **  \event <tt> 'Verify ATS Table error: invalid length: buf index = \%d, cmd
 *num = \%d, pkt len = \%d' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'Verify ATS Table error: buffer overflow: buf index = \%d, cmd
 *num = \%d, pkt len = \%d' </tt>
 **  \event <tt> 'Verify ATS Table error: buffer overflow: buf index = \%d, cmd
 *num = \%d, pkt len = \%d' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'Verify ATS Table error: dup cmd number: buf index = \%d, cmd
 *num = \%d, dup index = \%d' </tt>
 **  \event <tt> 'Verify ATS Table error: dup cmd number: buf index = \%d, cmd
 *num = \%d, dup index = \%d' </tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'RTS table manage process error: RTS = \%d, Result = 0x\%X'
 *</tt>
 **  \event <tt> 'RTS table manage process error: RTS = \%d, Result = 0x\%X'
 *</tt>
 **
 **  \par Type: ERROR
 **
...
/** \brief <tt> 'ATS table manage process error: ATS = \%d, Result = 0x\%X'
 *</tt>
 **  \event <tt> 'ATS table manage process error: ATS = \%d, Result = 0x\%X'
 *</tt>
 **
 **  \par Type: ERROR
 **
