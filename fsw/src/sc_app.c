 ...
 **   Arguements in CFE_EVS_SendEvent causing format warnings have been
 *explicitly cast to (unsigned int) and (int) same as cFE.
 ...
 **   During init when loading default RTS tables, increment the RTS_Count to
 *correctly report the number loaded.
 ...
 **   Delete enable auto-exec RTS from load defaults, add select auto-exec RTS
 *per boot type
 ...
 **   Update list of included header files, add SC_GetTableAddresses(), add
...
void SC_AppMain(void) {
    ...
    while (CFE_ES_RunLoop(&RunStatus)) {
        ...
        Result = CFE_SB_RcvMsg(&SC_OperData.MsgPtr, SC_OperData.CmdPipe,
                               CFE_SB_PEND_FOREVER);
...
int32 SC_AppInit(void) {
    ...
    Result = CFE_SB_CreatePipe(&SC_OperData.CmdPipe, SC_PIPE_DEPTH,
                               SC_CMD_PIPE_NAME);
    if (Result != CFE_SUCCESS) {
    ...
    CFE_EVS_SendEvent(SC_INIT_INF_EID, CFE_EVS_INFORMATION,
                      "SC Initialized. Version %d.%d.%d.%d", SC_MAJOR_VERSION,
...
int32 SC_RegisterAllTables(void) {
    ...
    TableOptions =
        (CFE_TBL_OPT_DEFAULT | CFE_TBL_OPT_SNGL_BUFFER | CFE_TBL_OPT_DUMP_ONLY);

    /* Register dump only RTS information table */
    TableSize = sizeof(SC_RtsInfoEntry_t) * SC_NUMBER_OF_RTS;
    Result = CFE_TBL_Register(&SC_OperData.RtsInfoHandle, SC_RTSINFO_TABLE_NAME,
                              TableSize, TableOptions, NULL);
    if (Result != CFE_SUCCESS) {
    ...
    Result =
        CFE_TBL_Register(&SC_OperData.RtsCtrlBlckHandle, SC_RTP_CTRL_TABLE_NAME,
                         TableSize, TableOptions, NULL);
    if (Result != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
    ...
    Result = CFE_TBL_Register(&SC_OperData.AtsInfoHandle, SC_ATSINFO_TABLE_NAME,
                              TableSize, TableOptions, NULL);
    if (Result != CFE_SUCCESS) {
    ...
    Result    = CFE_TBL_Register(&SC_OperData.AppendInfoHandle,
                              SC_APPENDINFO_TABLE_NAME, TableSize, TableOptions,
                              NULL);
    if (Result != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
    ...
    Result =
        CFE_TBL_Register(&SC_OperData.AtsCtrlBlckHandle, SC_ATS_CTRL_TABLE_NAME,
                         TableSize, TableOptions, NULL);
    if (Result != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
    ...
    for (i = 0; i < SC_NUMBER_OF_ATS; i++) {
        ...
        Result = CFE_TBL_Register(&SC_OperData.AtsCmdStatusHandle[i], TableName,
                                  TableSize, TableOptions, NULL);
        if (Result != CFE_SUCCESS) {
            CFE_EVS_SendEvent(SC_REGISTER_ATS_CMD_STATUS_TABLE_ERR_EID,
                              CFE_EVS_ERROR,
                              "ATS command status table register failed for "
                              "ATS %d, returned: 0x%08X",
    ...
    for (i = 0; i < SC_NUMBER_OF_RTS; i++) {
        ...
        Result = CFE_TBL_Register(&SC_OperData.RtsTblHandle[i], TableName,
                                  TableSize, TableOptions, SC_ValidateRts);
        if (Result != CFE_SUCCESS) {
            CFE_EVS_SendEvent(
    ...
    for (i = 0; i < SC_NUMBER_OF_ATS; i++) {
        ...
        Result = CFE_TBL_Register(&SC_OperData.AtsTblHandle[i], TableName,
                                  TableSize, TableOptions, SC_ValidateAts);
        if (Result != CFE_SUCCESS) {
            CFE_EVS_SendEvent(
    ...
    Result =
        CFE_TBL_Register(&SC_OperData.AppendTblHandle, SC_APPEND_TABLE_NAME,
                         TableSize, TableOptions, SC_ValidateAppend);
    if (Result != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
...
int32 SC_GetDumpTablePointers(void) {
    ...
    Result = CFE_TBL_GetAddress((void **)&SC_OperData.RtsInfoTblAddr,
                                SC_OperData.RtsInfoHandle);
    if (Result != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
    ...
    Result = CFE_TBL_GetAddress((void **)&SC_OperData.RtsCtrlBlckAddr,
    ...
    Result = CFE_TBL_GetAddress((void **)&SC_OperData.AtsInfoTblAddr,
    ...
    Result = CFE_TBL_GetAddress((void **)&SC_OperData.AppendInfoTblAddr,
                                SC_OperData.AppendInfoHandle);
    if (Result != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
            SC_GET_ADDRESS_APPEND_INFO_ERR_EID, CFE_EVS_ERROR,
    ...
    Result = CFE_TBL_GetAddress((void **)&SC_OperData.AtsCtrlBlckAddr,
    ...
    for (i = 0; i < SC_NUMBER_OF_ATS; i++) {
        Result =
            CFE_TBL_GetAddress((void **)&SC_OperData.AtsCmdStatusTblAddr[i],
                               SC_OperData.AtsCmdStatusHandle[i]);
        if (Result != CFE_SUCCESS) {
            CFE_EVS_SendEvent(SC_GET_ADDRESS_ATS_CMD_STAT_ERR_EID,
                              CFE_EVS_ERROR,
                              "ATS Cmd Status table for ATS %d failed Getting "
                              "Address, returned: 0x%08X",
                              i + 1, (unsigned int)Result);
            return (Result);
        }
    }
...
int32 SC_GetLoadTablePointers(void) {
    ...
    for (i = 0; i < SC_NUMBER_OF_ATS; i++) {
        Result = CFE_TBL_GetAddress((void **)&SC_OperData.AtsTblAddr[i],
                                    SC_OperData.AtsTblHandle[i]);

        if ((Result != CFE_TBL_ERR_NEVER_LOADED) &&
            (Result != CFE_TBL_INFO_UPDATED)) {
            CFE_EVS_SendEvent(
                SC_GET_ADDRESS_ATS_ERR_EID, CFE_EVS_ERROR,
                "ATS table %d failed Getting Address, returned: 0x%08X", i + 1,
                (unsigned int)Result);
            return (Result);
        }
    }
    ...
    for (i = 0; i < SC_NUMBER_OF_RTS; i++) {
        Result = CFE_TBL_GetAddress((void **)&SC_OperData.RtsTblAddr[i],
                                    SC_OperData.RtsTblHandle[i]);

        if ((Result != CFE_TBL_ERR_NEVER_LOADED) &&
            (Result != CFE_TBL_INFO_UPDATED)) {
            CFE_EVS_SendEvent(
                SC_GET_ADDRESS_RTS_ERR_EID, CFE_EVS_ERROR,
                "RTS table %d failed Getting Address, returned: 0x%08X", i + 1,
                (unsigned int)Result);
            return (Result);
        }
...
void SC_LoadDefaultTables(void) {
    ...
    */
    for (RtsIndex = 0; RtsIndex < SC_NUMBER_OF_RTS; RtsIndex++) {
        ...
        snprintf(TableName, sizeof(TableName), "%s%03d.tbl", SC_RTS_FILE_NAME,
                 (int)(RtsIndex + 1));
        ...
        if (FileDesc >= 0) {
            ...
            if (CFE_TBL_Load(SC_OperData.RtsTblHandle[RtsIndex],
                             CFE_TBL_SRC_FILE, TableName) == CFE_SUCCESS) {
                RtsCount++;
            }
        }
    }
...
void SC_RegisterManageCmds(void) {
    ...
    CFE_TBL_NotifyByMessage(SC_OperData.RtsInfoHandle, SC_CMD_MID,
                            SC_MANAGE_TABLE_CC, SC_TBL_ID_RTS_INFO);

    /* Register for RTS control block table manage request commands */
    CFE_TBL_NotifyByMessage(SC_OperData.RtsCtrlBlckHandle, SC_CMD_MID,
                            SC_MANAGE_TABLE_CC, SC_TBL_ID_RTP_CTRL);

    /* Register for ATS info table manage request commands */
    CFE_TBL_NotifyByMessage(SC_OperData.AtsInfoHandle, SC_CMD_MID,
                            SC_MANAGE_TABLE_CC, SC_TBL_ID_ATS_INFO);

    /* Register for ATS Append info table manage request commands */
    CFE_TBL_NotifyByMessage(SC_OperData.AppendInfoHandle, SC_CMD_MID,
                            SC_MANAGE_TABLE_CC, SC_TBL_ID_APP_INFO);

    /* Register for ATS control block table manage request commands */
    CFE_TBL_NotifyByMessage(SC_OperData.AtsCtrlBlckHandle, SC_CMD_MID,
                            SC_MANAGE_TABLE_CC, SC_TBL_ID_ATP_CTRL);

    /* Register for ATS Append table manage request commands */
    CFE_TBL_NotifyByMessage(SC_OperData.AppendTblHandle, SC_CMD_MID,
                            SC_MANAGE_TABLE_CC, SC_TBL_ID_APPEND);

    for (i = 0; i < SC_NUMBER_OF_ATS; i++) {
        /* Register for ATS cmd status table manage request commands */
        CFE_TBL_NotifyByMessage(SC_OperData.AtsCmdStatusHandle[i], SC_CMD_MID,
                                SC_MANAGE_TABLE_CC, SC_TBL_ID_ATS_CMD_0 + i);

        /* Register for ATS table manage request commands */
        CFE_TBL_NotifyByMessage(SC_OperData.AtsTblHandle[i], SC_CMD_MID,
                                SC_MANAGE_TABLE_CC, SC_TBL_ID_ATS_0 + i);
    }

    for (i = 0; i < SC_NUMBER_OF_RTS; i++) {
        /* Register for RTS table manage request commands */
        CFE_TBL_NotifyByMessage(SC_OperData.RtsTblHandle[i], SC_CMD_MID,
                                SC_MANAGE_TABLE_CC, SC_TBL_ID_RTS_0 + i);
    }
