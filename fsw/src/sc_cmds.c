...
**   Function Prototype for SC_ProcessCommand Needs to be Moved to .h File With
...
**   Support cFE Table Services notify commands for pending dump or validate,
*etc.
**   Revision 1.10 2010/09/28 10:42:28EDT lwalling
**   Update list of included header files, add table manage cmd handler
*functions
...
void SC_ProcessAtpCmd(void) {
    ...
    if ((!SC_CompareAbsTime(SC_AppData.NextCmdTime[SC_ATP],
         ...
         */
        AtsIndex =
            SC_OperData.AtsCtrlBlckAddr->AtsNumber - 1; /* remember 0..1 */
        ...
        Entry      = (SC_AtsEntryHeader_t *)&SC_OperData
                    .AtsTblAddr[AtsIndex][EntryIndex];
        CmdPtr = (CFE_SB_MsgPtr_t)Entry->CmdHeader;

        /*
         ** Make sure the command has not been executed, skipped or has any
         *other bad status
         */
        if (SC_OperData.AtsCmdStatusTblAddr[AtsIndex][CmdIndex] == SC_LOADED) {
            ...
            if (Entry->CmdNumber == (CmdIndex + 1)) {
                ...
                if (CFE_SB_ValidateChecksum(CmdPtr) == TRUE) {
                     ...
                     **  if it is, then execute the command now instead of
                     *sending
                     **  it out on the Software Bus (this is the only exception
                     *to
                    ...
                    if (CFE_SB_GetMsgId(CmdPtr) == SC_CMD_MID &&
                        ...
                        if (SC_InlineSwitch()) {
                             ...
                             */
                            SC_OperData
                                .AtsCmdStatusTblAddr[AtsIndex][CmdIndex] =
                                SC_EXECUTED;
                            SC_AppData.AtsCmdCtr++;
                        } else { /* the switch failed for some reason */

                            SC_OperData
                                .AtsCmdStatusTblAddr[AtsIndex][CmdIndex] =
                                SC_FAILED_DISTRIB;
                            SC_AppData.AtsCmdErrCtr++;
                            SC_AppData.LastAtsErrSeq =
                                SC_OperData.AtsCtrlBlckAddr->AtsNumber;
                            SC_AppData.LastAtsErrCmd =
                        ...
                        if (Result == CFE_SUCCESS) {
                        ...
                        } else { /* the command had Software Bus problems */
                            SC_OperData
                                .AtsCmdStatusTblAddr[AtsIndex][CmdIndex] =
                                SC_FAILED_DISTRIB;
                            SC_AppData.AtsCmdErrCtr++;
                            ...
                            CFE_EVS_SendEvent(
                                SC_ATS_DIST_ERR_EID, CFE_EVS_ERROR,
                                "ATS Command Distribution Failed, Cmd Number: "
                                "%d, SB returned: 0x%08X",
                                Entry->CmdNumber, (unsigned int)Result);

                            if (SC_OperData.AtsCtrlBlckAddr->AtsNumber ==
                                SC_ATSA)
                                TempAtsChar = 'A';
                            else if (SC_OperData.AtsCtrlBlckAddr->AtsNumber ==
                                     SC_ATSB)
                                TempAtsChar = 'B';
                ...
                } else { /* the checksum failed */
                    /*
                     ** Send an event message to report the invalid command
                     *status
                     */
                    CFE_EVS_SendEvent(
                        SC_ATS_CHKSUM_ERR_EID, CFE_EVS_ERROR,
                    ...
                    if (SC_AppData.ContinueAtsOnFailureFlag ==
                        FALSE) { /* Stop ATS execution */
                        /*
            ...
            } else { /* the command number does not match */
                /*
        ...
        } else /* command isn't marked as loaded */
        {
            ...
            /* Do Not Mark this ATS for abortion. The command could be marked as
               EXECUTED if we alerady jumped back in time */

        } /* end if */

        if (AbortATS == TRUE) {
            CFE_EVS_SendEvent(SC_ATS_ABT_ERR_EID, CFE_EVS_ERROR,
                              "ATS %c Aborted", TempAtsChar);
...
void SC_ProcessRtpCommand(void) {
    ...
    if ((SC_AppData.NextCmdTime[SC_AppData.NextProcNumber] <=
         SC_AppData.CurrentTime) &&
         ...
         */
        RtsEntryPtr =
            (SC_RtsEntryHeader_t *)&SC_OperData.RtsTblAddr[RtsNum][CmdOffset];
        ...
        if (CFE_SB_ValidateChecksum(CmdPtr) == TRUE) {
            ...
            } else { /* the software bus return code was bad */

                /*
                 ** Send an event message to report the invalid command status
                 */
                CFE_EVS_SendEvent(SC_RTS_DIST_ERR_EID, CFE_EVS_ERROR,
                                  "RTS %03d Command Distribution Failed: RTS "
                                  "Stopped. SB returned 0x%08X",
                                  (int)(RtsNum + 1), (unsigned int)Result);

                SC_AppData.RtsCmdErrCtr++;
                SC_OperData.RtsInfoTblAddr[RtsNum].CmdErrCtr++;
                SC_AppData.LastRtsErrSeq =
             ...
             ** Send an event message to report the invalid command status
             */
            CFE_EVS_SendEvent(SC_RTS_CHKSUM_ERR_EID, CFE_EVS_ERROR,
...
void SC_SendHkPacket(void) {
    ...
    SC_OperData.HkPacket.AtpCmdNumber =
        SC_OperData.AtsCtrlBlckAddr->CmdNumber + 1;
    SC_OperData.HkPacket.SwitchPendFlag =
        SC_OperData.AtsCtrlBlckAddr->SwitchPendFlag;
    ...
    SC_OperData.HkPacket.NumRtsActive =
    ...
    SC_OperData.HkPacket.ContinueAtsOnFailureFlag =
        SC_AppData.ContinueAtsOnFailureFlag;
...
void SC_ResetCountersCmd(CFE_SB_MsgPtr_t CmdPacket) {
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_NoArgsCmd_t))) {
        CFE_EVS_SendEvent(SC_RESET_DEB_EID, CFE_EVS_DEBUG,
                          "Reset counters command");
...
void SC_NoOpCmd(CFE_SB_MsgPtr_t CmdPacket) {
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_NoArgsCmd_t))) {
        SC_AppData.CmdCtr++;
        CFE_EVS_SendEvent(SC_NOOP_INF_EID, CFE_EVS_INFORMATION,
                          "No-op command. Version %d.%d.%d.%d",
                          SC_MAJOR_VERSION, SC_MINOR_VERSION, SC_REVISION,
                          SC_MISSION_REV);
    }
} /* End SC_NoOpCmd */
...
void SC_ProcessRequest(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    switch (MessageID) {
        ...
        case SC_SEND_HK_MID:
            if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_NoArgsCmd_t))) {
                ...
                if (SC_AppData.AutoStartRTS != 0) {
                    ...
                    if (SC_OperData.RtsInfoTblAddr[SC_AppData.AutoStartRTS - 1]
                            .RtsStatus == SC_LOADED) {
                        SC_OperData.RtsInfoTblAddr[SC_AppData.AutoStartRTS - 1]
                            .DisabledFlag = FALSE;
                    }
        ...
        case SC_1HZ_WAKEUP_MID:
            ...
            do {
                ...
                if ((SC_AppData.NextProcNumber == SC_NONE) ||
                    (SC_AppData.NextCmdTime[SC_AppData.NextProcNumber] >
                     SC_AppData.CurrentTime)) {
        ...
        default:
            CFE_EVS_SendEvent(SC_MID_ERR_EID, CFE_EVS_ERROR,
...
void SC_ProcessCommand(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    switch (CommandCode) {
        ...
        default:
            CFE_EVS_SendEvent(SC_INVLD_CMD_ERR_EID, CFE_EVS_ERROR,
                              "Invalid Command Code: MID =  0x%04X CC =  %d",
                              CFE_SB_GetMsgId(CmdPacket), CommandCode);
            SC_AppData.CmdErrCtr++;
            break;
    } /* end switch */
...
void SC_TableManageCmd(CFE_SB_MsgPtr_t CmdPacket) {
    int32 ArrayIndex;
    int32 TableID =
        (int32)((CFE_TBL_NotifyCmd_t *)CmdPacket)->Payload.Parameter;
...
void SC_ManageRtsTable(int32 ArrayIndex) {
    ...
    if (Result == CFE_TBL_INFO_UPDATED) {
        ...
        SC_LoadRts(ArrayIndex);
               ...
               (Result != CFE_TBL_ERR_NEVER_LOADED)) {
