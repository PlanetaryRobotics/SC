...
**   Add new command handlers -- SC_StartRtsGrpCmd(), SC_StopRtsGrpCmd(),
*SC_DisableGrpCmd(), SC_EnableGrpCmd().
...
void SC_StartRtsCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_RtsCmd_t))) {
        ...
        if (RtsIndex < SC_NUMBER_OF_RTS) {
            ...
            if (SC_OperData.RtsInfoTblAddr[RtsIndex].DisabledFlag == FALSE) {
                ...
                if (SC_OperData.RtsInfoTblAddr[RtsIndex].RtsStatus ==
                    SC_LOADED) {
                    /*
                     ** Check the command length
                     */
                    RtsEntryPtr =
                        (SC_RtsEntryHeader_t *)SC_OperData.RtsTblAddr[RtsIndex];
                    ...
                    if (CmdLength >= SC_PACKET_MIN_SIZE &&
                        CmdLength <= SC_PACKET_MAX_SIZE) {
                        /*
                        ...
                        /*
                         ** Last, Increment some global counters associated with
                         *the
                         ** starting of the RTS
                         */
                        ...
                        if (((SC_RtsCmd_t *)CmdPacket)->RtsId <=
                            SC_LAST_RTS_WITH_EVENTS) {
                            CFE_EVS_SendEvent(
                                SC_RTS_START_INF_EID, CFE_EVS_INFORMATION,
                                "RTS Number %03d Started",
                                ((SC_RtsCmd_t *)CmdPacket)->RtsId);
                        } else {
                            CFE_EVS_SendEvent(
                                SC_STARTRTS_CMD_DBG_EID, CFE_EVS_DEBUG,
                                "Start RTS #%d command",
                                ((SC_RtsCmd_t *)CmdPacket)->RtsId);
                        }
                    } else { /* the length field of the 1st cmd was bad */
                        CFE_EVS_SendEvent(
                            SC_STARTRTS_CMD_INVLD_LEN_ERR_EID, CFE_EVS_ERROR,
                            "Start RTS %03d Rejected: Invld Len Field for 1st "
                            "Cmd in Sequence. Invld Cmd Length = %d",
                            ((SC_RtsCmd_t *)CmdPacket)->RtsId, CmdLength);

                        SC_AppData.CmdErrCtr++;
                        SC_AppData.RtsActiveErrCtr++;

                    }    /* end if - check command number */
                } else { /* Cannot use the RTS now */

                    CFE_EVS_SendEvent(
                        SC_STARTRTS_CMD_NOT_LDED_ERR_EID, CFE_EVS_ERROR,
                        "Start RTS %03d Rejected: RTS Not Loaded or In Use, "
                        "Status: %d",
                        ((SC_RtsCmd_t *)CmdPacket)->RtsId,
                        SC_OperData.RtsInfoTblAddr[RtsIndex].RtsStatus);
        ...
        } else { /* the rts id is invalid */
            CFE_EVS_SendEvent(SC_STARTRTS_CMD_INVALID_ERR_EID, CFE_EVS_ERROR,
    ...
    } else { /* the command length is invalid */
        SC_AppData.RtsActiveErrCtr++;
    }
...
void SC_StartRtsGrpCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_RtsGrpCmd_t))) {
        ...
        if ((FirstIndex < SC_NUMBER_OF_RTS) && (LastIndex < SC_NUMBER_OF_RTS) &&
            (FirstIndex <= LastIndex)) {
            for (RtsIndex = FirstIndex; RtsIndex <= LastIndex; RtsIndex++) {
                /* make sure that RTS is not disabled, empty or executing */
                if ((SC_OperData.RtsInfoTblAddr[RtsIndex].DisabledFlag ==
                     FALSE) &&
                    (SC_OperData.RtsInfoTblAddr[RtsIndex].RtsStatus ==
                     SC_LOADED)) {
                    /* initialize the RTS info table entry */
                    ...
                    SC_OperData.RtsInfoTblAddr[RtsIndex].NextCommandTime =
                        SC_ComputeAbsTime(((SC_RtsEntryHeader_t *)
                                               SC_OperData.RtsTblAddr[RtsIndex])
                                              ->TimeTag);
...
void SC_StopRtsCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_RtsCmd_t))) {
        ...
        if (RtsIndex < SC_NUMBER_OF_RTS) {
            ...
            CFE_EVS_SendEvent(SC_STOPRTS_CMD_INF_EID, CFE_EVS_INFORMATION,
                              "RTS %03d Aborted",
                              ((SC_RtsCmd_t *)CmdPacket)->RtsId);
        } else { /* the specified RTS is invalid */

            /* the rts id is invalid */
            CFE_EVS_SendEvent(SC_STOPRTS_CMD_ERR_EID, CFE_EVS_ERROR,
...
void SC_StopRtsGrpCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_RtsGrpCmd_t))) {
        ...
        if ((FirstIndex < SC_NUMBER_OF_RTS) && (LastIndex < SC_NUMBER_OF_RTS) &&
            ...
            /* success */
            CFE_EVS_SendEvent(
                SC_STOPRTSGRP_CMD_INF_EID, CFE_EVS_INFORMATION,
...
void SC_DisableRtsCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_RtsCmd_t))) {
        ...
        if (RtsIndex < SC_NUMBER_OF_RTS) {
            ...
            CFE_EVS_SendEvent(SC_DISABLE_RTS_DEB_EID, CFE_EVS_DEBUG,
                              "Disabled RTS %03d",
                              ((SC_RtsCmd_t *)CmdPacket)->RtsId);
        } else { /* it is not a valid RTS id */
            CFE_EVS_SendEvent(SC_DISRTS_CMD_ERR_EID, CFE_EVS_ERROR,
...
void SC_DisableRtsGrpCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_RtsGrpCmd_t))) {
        ...
        if ((FirstIndex < SC_NUMBER_OF_RTS) && (LastIndex < SC_NUMBER_OF_RTS) &&
            ...
            /* success */
            CFE_EVS_SendEvent(
                SC_DISRTSGRP_CMD_INF_EID, CFE_EVS_INFORMATION,
...
void SC_EnableRtsCmd(CFE_SB_MsgPtr_t CmdPacket) {
    ...
    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_RtsCmd_t))) {
        ...
        if (RtsIndex < SC_NUMBER_OF_RTS) {
            ...
            CFE_EVS_SendEvent(SC_ENABLE_RTS_DEB_EID, CFE_EVS_DEBUG,
                              "Enabled RTS %03d",
                              ((SC_RtsCmd_t *)CmdPacket)->RtsId);

        } else { /* it is not a valid RTS id */
            CFE_EVS_SendEvent(SC_ENARTS_CMD_ERR_EID, CFE_EVS_ERROR,
