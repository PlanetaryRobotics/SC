 ...
 **   Change function and structure names from HkStatus to HkPacket, remove
 references to CDS
 ...
 **   Cleanup function return code definitions, move dup cmd test array to
 global structure
...
/** \{ */
#define SC_BYTES_IN_WORD \
    2 /**< \brief Words are used to define table lengths   */
/** \} */
...
typedef struct {
    ...
    CFE_TBL_Handle_t AtsTblHandle[SC_NUMBER_OF_ATS]; /**< \brief Table handles
                                                        for all ATS tables    */
    uint16* AtsTblAddr[SC_NUMBER_OF_ATS]; /**< \brief Table Addresses for all
                                             ATS tables  */

    CFE_TBL_Handle_t
        AppendTblHandle;   /**< \brief Table handle for Append ATS table   */
    uint16* AppendTblAddr; /**< \brief Table Address for Append ATS table  */

    CFE_TBL_Handle_t RtsTblHandle[SC_NUMBER_OF_RTS]; /**< \brief Table handles
                                                        for all RTS tables    */
    uint16* RtsTblAddr[SC_NUMBER_OF_RTS]; /**< \brief Table addresses for all
                                             RTS tables  */

    CFE_TBL_Handle_t
        AtsInfoHandle; /**< \brief Table handle the for ATS Info Table */
    SC_AtsInfoTable_t*
        AtsInfoTblAddr; /**< \brief Table address for the ATS Info Table*/

    CFE_TBL_Handle_t
        AppendInfoHandle; /**< \brief Table handle for Append Info Table  */
    SC_AtsInfoTable_t*
        AppendInfoTblAddr; /**< \brief Table address for Append Info Table */

    CFE_TBL_Handle_t
        RtsInfoHandle; /**< \brief Table handle for RTS Info Table     */
    SC_RtsInfoEntry_t*
        RtsInfoTblAddr; /**< \brief Table address for RTS INfo Table    */

    CFE_TBL_Handle_t
        RtsCtrlBlckHandle; /**< \brief Table handle for the RTP ctrl block */
    SC_RtpControlBlock_t*
        RtsCtrlBlckAddr; /**< \brief Table address for the RTP ctrl block*/

    CFE_TBL_Handle_t
        AtsCtrlBlckHandle; /**< \brief Table handle for the ATP ctrl block */
    SC_AtpControlBlock_t*
        AtsCtrlBlckAddr; /**< \brief Table address for the ATP ctrl block*/

    CFE_TBL_Handle_t
        AtsCmdStatusHandle[SC_NUMBER_OF_ATS]; /**< \brief ATS Cmd Status table
                                                 handle     */
    uint8* AtsCmdStatusTblAddr[SC_NUMBER_OF_ATS]; /**< \brief ATS Cmd Status
                                                     table address    */

    int32 AtsDupTestArray[SC_MAX_ATS_CMDS]; /**< \brief ATS test for duplicate
                                               cmd numbers  */
...
typedef struct {
    ...
    /**< \brief If an Ats command fails checksum the ATS execution will continue
       if This flag is set to TRUE and will stop if this flag is set to FALSE*/
    ...
    SC_AbsTimeTag_t NextCmdTime[2]; /**< \brief The overall next command time  0
                                       - ATP, 1- RTP*/
    ...
    uint16 RtsActiveErrCtr; /**< \brief Increments when an attempt to start an
                               RTS fails */
    uint16 RtsActiveCtr; /**< \brief Increments when an RTS is started without
                            error */
    uint16 AtsCmdCtr; /**< \brief Total ATS cmd counter counts commands sent by
                         the ATS */
    uint16 AtsCmdErrCtr; /**< \brief Total ATS cmd Error ctr command errors in
                            the ATS */
    uint16 RtsCmdCtr; /**< \brief Counts TOTAL rts commands that were sent out
                         from ALL active RTSs */
    uint16 RtsCmdErrCtr;  /**< \brief Counts TOTAL number of errs from ALL RTSs
                             that are active */
    uint16 LastAtsErrSeq; /**< \brief Last ATS Errant Sequence Num Values: 1 or
                             2 */
    ...
    /**< \brief The OFFSET in the RTS buffer of the command. It will be a WORD
       value i.e.  1st command had an error, this value would be 0, if the 2nd
       command started at int8 10 in the buffer, this value would be 5. */

    uint16 AppendCmdArg;     /**< \brief ATS selection argument from most recent
                                Append ATS command */
    uint16 AppendEntryCount; /**< \brief Number of cmd entries in current Append
                                ATS table */
    uint16 AppendWordCount;  /**< \brief Size of cmd entries in current Append
                                ATS table */
    uint16
        AppendLoadCount; /**< \brief Total number of Append ATS table loads */
