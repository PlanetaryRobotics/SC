...
void SC_GetNextRtsCommand(void) {
    ...
    if ((SC_OperData.RtsCtrlBlckAddr->RtsNumber > 0) &&
        ...
        if (SC_OperData.RtsInfoTblAddr[RtsNum].RtsStatus == SC_EXECUTING) {
            ...
            RtsEntryPtr = (SC_RtsEntryHeader_t *)&SC_OperData
                              .RtsTblAddr[RtsNum][CmdOffset];
            ...
            /* If at least the header for a command plus the RTS header can fit
             * in the buffer */
            if (CmdOffset < (SC_RTS_BUFF_SIZE - (sizeof(SC_RtsEntryHeader_t) /
                                                 SC_BYTES_IN_WORD))) {
                /*
                 ...
                 */
                CmdLength =
                    CFE_SB_GetTotalMsgLength(CmdPtr) + SC_RTS_HEADER_SIZE;

                /*
                 ** Check to see if the command length is less than the size of
                 *a header.
                ...
                if ((CmdLength - SC_RTS_HEADER_SIZE) >= (SC_PACKET_MIN_SIZE)) {
                     ...
                     */
                    if ((CmdLength - SC_RTS_HEADER_SIZE) <=
                        SC_PACKET_MAX_SIZE) {
                        /*
                         ...
                         ** (plus 1 to round byte len up to word len)
                         */
                        if (CmdOffset + ((CmdLength + 1) / SC_BYTES_IN_WORD) <=
                            SC_RTS_BUFF_SIZE) {
                            /*
                             ** Everything passed!
                            ...
                            /*
                             ** Update the appropriate RTS info table current
                             *command pointer
                             */
                            SC_OperData.RtsInfoTblAddr[RtsNum].NextCommandPtr =
                                CmdOffset;

                        } else { /* the command runs past the end of the buffer
                                  */

                            /*
                             ** Having a command that runs off of the end of the
                             *buffer
                            ...
                            SC_KillRts(RtsNum);
                            CFE_EVS_SendEvent(
                                SC_RTS_LNGTH_ERR_EID, CFE_EVS_ERROR,
                                "Cmd Runs passed end of table, RTS %03d "
                                "Aborted",
                                SC_OperData.RtsCtrlBlckAddr->RtsNumber);
                    ...
                    } else { /* the command length is too large */
                        ...
                        SC_KillRts(RtsNum);
                        CFE_EVS_SendEvent(
                            SC_RTS_CMD_LNGTH_ERR_EID, CFE_EVS_ERROR,
                            "Invalid Length Field in RTS Command, RTS %03d "
                            "Aborted. Length: %d, Max: %d",
                            SC_OperData.RtsCtrlBlckAddr->RtsNumber,
                            (CmdLength - SC_RTS_HEADER_SIZE),
                            SC_PACKET_MAX_SIZE);

                    }    /* end if the command length is invalid */
                } else { /* The command length is zero indicating no more cmds
                          */
                    ...
                    if ((RtsNum + 1) <= SC_LAST_RTS_WITH_EVENTS) {
                        CFE_EVS_SendEvent(
                            SC_RTS_COMPL_INF_EID, CFE_EVS_INFORMATION,
                            "RTS %03d Execution Completed", RtsNum + 1);
                    }
                }
            } else { /* The end of the RTS buffer has been reached... */

                /* Stop the RTS from executing */
                SC_KillRts(RtsNum);
                if ((RtsNum + 1) <= SC_LAST_RTS_WITH_EVENTS) {
                    CFE_EVS_SendEvent(SC_RTS_COMPL_INF_EID, CFE_EVS_INFORMATION,
    ...
    if (SC_OperData.AtsCtrlBlckAddr->AtpState == SC_EXECUTING) {
        ...
        if (TimeIndex < SC_OperData.AtsInfoTblAddr[AtsIndex].NumberOfCommands) {
            ...
            SC_OperData.AtsCtrlBlckAddr->CmdNumber =
                SC_AppData.AtsTimeIndexBuffer[AtsIndex][TimeIndex];

            /* update the next command time */
            SC_AppData.NextCmdTime[SC_ATP] = SC_GetAtsEntryTime(
                (SC_AtsEntryHeader_t *)&SC_OperData.AtsTblAddr
                    [AtsIndex]
                    [SC_AppData.AtsCmdIndexBuffer
                         [AtsIndex][SC_OperData.AtsCtrlBlckAddr->CmdNumber]]);
        } else { /* the end is near... of the ATS buffer that is */

            /* stop the ATS */
            SC_KillAts();
            CFE_EVS_SendEvent(SC_ATS_COMPL_INF_EID, CFE_EVS_INFORMATION,
