...
**   Changed local storage of Append ATS table use from bytes to words, wrote
...
**   Create function to verify one ATS entry, create function to verify Append
*ATS table data
...
**   Fixed packet length calculation to support odd byte length ATS and RTS
*commands
...
void SC_LoadAts(uint16 AtsIndex) {
    ...
    while (StillProcessing) {
        ...
        if (AtsEntryIndex < SC_ATS_BUFF_SIZE) {
            ...
            AtsCmdNum =
                ((SC_AtsEntryHeader_t *)&AtsTablePtr[AtsEntryIndex])->CmdNumber;
            ...
            /* make sure the CmdPtr can fit in a whole Ats Cmd Header at the
               very least */
            else if (AtsEntryIndex >
                     (SC_ATS_BUFF_SIZE -
                      (sizeof(SC_AtsEntryHeader_t) / SC_BYTES_IN_WORD))) {
            ...
            /* has not already been loaded                     */
                     ...
                     SC_OperData.AtsCmdStatusTblAddr[AtsIndex][AtsCmdNum - 1] ==
                         SC_EMPTY) {
                /* get a pointer to the ats command in the table */
                AtsEntryPtr =
                    (SC_AtsEntryHeader_t *)&AtsTablePtr[AtsEntryIndex];
                ...
                if (CFE_SB_GetTotalMsgLength(AtsCmd) >= SC_PACKET_MIN_SIZE &&
                    ...
                    /* get the length of the entry in WORDS (plus 1 to round
                     * byte len up to word len) */
                    AtsEntryWords = (CFE_SB_GetTotalMsgLength(AtsCmd) + 1 +
                                     SC_ATS_HEADER_SIZE) /
                                    SC_BYTES_IN_WORD;
                    ...
                    if (AtsEntryIndex + AtsEntryWords <= SC_ATS_BUFF_SIZE) {
                        ...
                        SC_AppData.AtsCmdIndexBuffer[AtsIndex][AtsCmdNum - 1] =
                            AtsEntryIndex;

                        /* set the command status to loaded in the command
                         * status table */
                        SC_OperData
                            .AtsCmdStatusTblAddr[AtsIndex][AtsCmdNum - 1] =
                            SC_LOADED;
                    ...
                    } else { /* the command runs off the end of the buffer */
                        Result          = SC_ERROR;
                        StillProcessing = FALSE;
                    }    /* end if */
                } else { /* the command length was invalid */
                    Result          = SC_ERROR;
                    StillProcessing = FALSE;
                }    /* end if */
            } else { /* the cmd number is invalid */
                Result          = SC_ERROR;
                StillProcessing = FALSE;
            } /* end if */
        } else {
            ...
            } else { /* the pointer is over the end of the buffer */
    ...
    if ((Result == CFE_SUCCESS) &&
        ...
        SC_OperData.AtsInfoTblAddr[AtsIndex].AtsSize =
            AtsEntryIndex; /* size in WORDS */
...
void SC_Insert(uint16 AtsIndex, uint32 NewCmdIndex, uint32 ListLength) {
    ...
    if (ListLength > 0) {
        /* first get the entry index in the selected ATS table for the new
         * command */
...
boolean SC_ParseRts(uint16 Buffer[]) {
    ...
    while (Error == FALSE && Done == FALSE) {
         ...
         */
        if (i <= (SC_RTS_BUFF_SIZE -
                  (sizeof(SC_RtsEntryHeader_t) / SC_BYTES_IN_WORD))) {
            /*
            ...
            if ((RtsEntryPtr->TimeTag == 0) && (CFE_SB_GetMsgId(RtsCmd) == 0)) {
                ...
                CFE_EVS_SendEvent(SC_RTS_INVLD_MID_ERR_EID, CFE_EVS_ERROR,
                ...
                if (CFE_SB_GetTotalMsgLength(RtsCmd) < SC_PACKET_MIN_SIZE ||
                    ...
                    CFE_EVS_SendEvent(
                        ...
                        "RTS cmd loaded with invalid length at %d, len: %d", i,
                        CFE_SB_GetTotalMsgLength(RtsCmd));

                    Error = TRUE; /* Length error */

                } else if ((i + ((RtsCmdSize + 1) / SC_BYTES_IN_WORD)) >
                           SC_RTS_BUFF_SIZE) {
                    CFE_EVS_SendEvent(SC_RTS_LEN_BUFFER_ERR_EID, CFE_EVS_ERROR,
                                      "RTS cmd at %d runs off end of buffer",
                                      i);
                    Error =
                        TRUE; /* command runs off of the end of the buffer */
                } else if ((i + ((RtsCmdSize + 1) / SC_BYTES_IN_WORD)) ==
                           SC_RTS_BUFF_SIZE) {
                    Done = TRUE;
                } else { /* command fits in buffer */

                    i += ((RtsCmdSize + 1) /
                          SC_BYTES_IN_WORD); /* remember 'i' is expressed in
                                                words */

                } /* end if */

            }    /* endif */
        } else { /* command does not fit in the buffer */
            ...
            if (Buffer[i] == 0) {
                ...
                CFE_EVS_SendEvent(SC_RTS_LEN_TOO_LONG_ERR_EID, CFE_EVS_ERROR,
...
void SC_UpdateAppend(void) {
    ...
    while (StillProcessing) {
        if (EntryIndex >= SC_APPEND_BUFF_SIZE) {
            ...
            Entry =
                (SC_AtsEntryHeader_t *)&SC_OperData.AppendTblAddr[EntryIndex];

            if ((Entry->CmdNumber == 0) ||
                (Entry->CmdNumber > SC_MAX_ATS_CMDS)) {
                ...
                if ((CommandBytes < SC_PACKET_MIN_SIZE) ||
                    (CommandBytes > SC_PACKET_MAX_SIZE)) {
                    /* Entry command packet must have a valid length */
                    StillProcessing = FALSE;
                } else if ((EntryIndex + SC_ATS_HDR_NOPKT_WORDS +
                            CommandWords) > SC_APPEND_BUFF_SIZE) {
                    /* Entry command packet must fit within ATS append table
                     * buffer */
                    StillProcessing = FALSE;
                } else {
    ...
    CFE_EVS_SendEvent(SC_UPDATE_APPEND_EID, CFE_EVS_INFORMATION,
                      "Update Append ATS Table: load count = %d, command count "
                      "= %d, byte count = %d",
                      SC_AppData.AppendLoadCount, (int)EntryCount,
                      (int)EntryIndex * 2);
    return;
    ...
    if (EntryIndex >= BufferWords) {
        ...
        */
        ...
        CFE_EVS_SendEvent(SC_VERIFY_ATS_NUM_ERR_EID, CFE_EVS_ERROR,
                          "Verify ATS Table error: invalid command number: buf "
                          "index = %d, cmd num = %d",
        ...
        if ((CommandBytes < SC_PACKET_MIN_SIZE) ||
            ...
            CFE_EVS_SendEvent(SC_VERIFY_ATS_PKT_ERR_EID, CFE_EVS_ERROR,
                              ...
                              "index = %d, cmd num = %d, pkt len = %d",
                              (int)EntryIndex, Entry->CmdNumber,
                              (int)CommandBytes);
        } else if ((EntryIndex + SC_ATS_HDR_NOPKT_WORDS + CommandWords) >
                   BufferWords) {
            ...
            CFE_EVS_SendEvent(SC_VERIFY_ATS_BUF_ERR_EID, CFE_EVS_ERROR,
                              ...
                              (int)EntryIndex, Entry->CmdNumber,
                              (int)CommandBytes);
        } else if (SC_OperData.AtsDupTestArray[Entry->CmdNumber - 1] !=
                   SC_DUP_TEST_UNUSED) {
            /* Entry with duplicate command number is invalid */
            Result = SC_ERROR;

            CFE_EVS_SendEvent(
                SC_VERIFY_ATS_DUP_ERR_EID, CFE_EVS_ERROR,
                "Verify ATS Table error: dup cmd number: buf index = %d, cmd "
                "num = %d, dup index = %d",
                (int)EntryIndex, Entry->CmdNumber,
