...
void SC_StartRtsCmd_Test_Nominal(void) {
    ...
    /* Set message size in order to satisfy if-statement after comment "Make
     * sure the command is big enough, but not too big" */
    Ut_CFE_SB_SetReturnCode(UT_CFE_SB_GETTOTALMSGLENGTH_INDEX,
                            sizeof(SC_RtsCmd_t), 1);
    ...
    UtAssert_True(
        ...
        "SC_OperData.RtsInfoTblAddr[RtsIndex].RtsStatus == SC_EXECUTING");
    UtAssert_True(SC_OperData.RtsInfoTblAddr[RtsIndex].CmdCtr == 0,
    ...
    UtAssert_True(SC_OperData.RtsInfoTblAddr[RtsIndex].UseCtr == 1,
                  "SC_OperData.RtsInfoTblAddr[RtsIndex].UseCtr == 1");

    UtAssert_True(SC_OperData.RtsCtrlBlckAddr->NumRtsActive == 1,
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_RTS_START_INF_EID, CFE_EVS_INFORMATION,
...
void SC_StartRtsCmd_Test_StartRtsNoEvents(void) {
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(SC_STARTRTS_CMD_DBG_EID, CFE_EVS_DEBUG,
                                       "Start RTS #5 command"),
                  "Start RTS #5 command");
...
void SC_StartRtsCmd_Test_InvalidCommandLength(void) {
    ...
    /* Verify results */
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_STARTRTS_CMD_INVLD_LEN_ERR_EID, CFE_EVS_ERROR,
                             "Start RTS 001 Rejected: Invld Len Field for 1st "
                             "Cmd in Sequence. Invld Cmd Length = 49159"),
        "Start RTS 001 Rejected: Invld Len Field for 1st Cmd in Sequence. "
        "Invld Cmd Length = 49159");
...
void SC_StartRtsCmd_Test_RtsNotLoadedOrInUse(void) {
    ...
    /* Verify results */
    UtAssert_True(SC_AppData.CmdErrCtr == 1,
                  "SC_OperData.RtsInfoTblAddr[RtsIndex].CmdErrCtr == 1");
    UtAssert_True(SC_AppData.RtsActiveErrCtr == 1,
                  "SC_OperData.RtsInfoTblAddr[RtsIndex].RtsActiveErrCtr == 1");

    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            SC_STARTRTS_CMD_NOT_LDED_ERR_EID, CFE_EVS_ERROR,
...
void SC_StartRtsCmd_Test_RtsDisabled(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_STARTRTS_CMD_DISABLED_ERR_EID, CFE_EVS_ERROR,
...
void SC_StartRtsCmd_Test_InvalidRtsId(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_STARTRTS_CMD_INVALID_ERR_EID, CFE_EVS_ERROR,
...
void SC_StartRtsGrpCmd_Test_Nominal(void) {
    ...
    /* Set message size so SC_VerifyCmdLength will return TRUE, to satisfy first
     * if-statement */
    Ut_CFE_SB_SetReturnCode(UT_CFE_SB_GETTOTALMSGLENGTH_INDEX,
                            sizeof(SC_RtsGrpCmd_t), 1);
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(
                      SC_STARTRTSGRP_CMD_INF_EID, CFE_EVS_INFORMATION,
...
void SC_StartRtsGrpCmd_Test_StartRtsGroupError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_STARTRTSGRP_CMD_ERR_EID, CFE_EVS_ERROR,
...
void SC_StopRtsCmd_Test_Nominal(void) {
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(SC_STOPRTS_CMD_INF_EID,
                                       CFE_EVS_INFORMATION, "RTS 001 Aborted"),
                  "RTS 001 Aborted");
...
void SC_StopRtsCmd_Test_InvalidRts(void) {
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(SC_STOPRTS_CMD_ERR_EID, CFE_EVS_ERROR,
...
void SC_DisableRtsCmd_Test_Nominal(void) {
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(SC_DISABLE_RTS_DEB_EID, CFE_EVS_DEBUG,
                                       "Disabled RTS 001"),
                  "Disabled RTS 001");
...
void SC_DisableRtsCmd_Test_InvalidRtsID(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_DISRTS_CMD_ERR_EID, CFE_EVS_ERROR,
...
void SC_EnableRtsCmd_Test_Nominal(void) {
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(SC_ENABLE_RTS_DEB_EID, CFE_EVS_DEBUG,
                                       "Enabled RTS 001"),
                  "Enabled RTS 001");
...
void SC_EnableRtsCmd_Test_InvalidRtsID(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_ENARTS_CMD_ERR_EID, CFE_EVS_ERROR,
...
void SC_EnableRtsGrpCmd_Test_Nominal(void) {
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(
                      SC_ENARTSGRP_CMD_INF_EID, CFE_EVS_INFORMATION,
...
void SC_EnableRtsGrpCmd_Test_Error(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_ENARTSGRP_CMD_ERR_EID, CFE_EVS_ERROR,
...
void SC_KillRts_Test(void) {
    ...
    UtAssert_True(
        ...
        "SC_OperData.RtsInfoTblAddr[RtsIndex].NextCommandTime == SC_MAX_TIME");
    UtAssert_True(SC_OperData.RtsCtrlBlckAddr->NumRtsActive == 0,
...
void SC_Rtsrq_Test_AddTestCases(void) {
    UtTest_Add(SC_StartRtsCmd_Test_Nominal, SC_Test_Setup, SC_Test_TearDown,
               "SC_StartRtsCmd_Test_Nominal");
    UtTest_Add(SC_StartRtsCmd_Test_StartRtsNoEvents, SC_Test_Setup,
               SC_Test_TearDown, "SC_StartRtsCmd_Test_StartRtsNoEvents");
    UtTest_Add(SC_StartRtsCmd_Test_InvalidCommandLength, SC_Test_Setup,
               SC_Test_TearDown, "SC_StartRtsCmd_Test_InvalidCommandLength");
    UtTest_Add(SC_StartRtsCmd_Test_RtsNotLoadedOrInUse, SC_Test_Setup,
               SC_Test_TearDown, "SC_StartRtsCmd_Test_RtsNotLoadedOrInUse");
    UtTest_Add(SC_StartRtsCmd_Test_RtsDisabled, SC_Test_Setup, SC_Test_TearDown,
               "SC_StartRtsCmd_Test_RtsDisabled");
    UtTest_Add(SC_StartRtsCmd_Test_InvalidRtsId, SC_Test_Setup,
               SC_Test_TearDown, "SC_StartRtsCmd_Test_InvalidRtsId");
    UtTest_Add(SC_StartRtsGrpCmd_Test_Nominal, SC_Test_Setup, SC_Test_TearDown,
               "SC_StartRtsGrpCmd_Test_Nominal");
    UtTest_Add(SC_StartRtsGrpCmd_Test_StartRtsGroupError, SC_Test_Setup,
               SC_Test_TearDown, "SC_StartRtsGrpCmd_Test_StartRtsGroupError");
    UtTest_Add(SC_StopRtsCmd_Test_Nominal, SC_Test_Setup, SC_Test_TearDown,
               "SC_StopRtsCmd_Test_Nominal");
    UtTest_Add(SC_StopRtsCmd_Test_InvalidRts, SC_Test_Setup, SC_Test_TearDown,
               "SC_StopRtsCmd_Test_InvalidRts");
    UtTest_Add(SC_StopRtsGrpCmd_Test_Nominal, SC_Test_Setup, SC_Test_TearDown,
               "SC_StopRtsGrpCmd_Test_Nominal");
    UtTest_Add(SC_StopRtsGrpCmd_Test_Error, SC_Test_Setup, SC_Test_TearDown,
               "SC_StopRtsGrpCmd_Test_Error");
    UtTest_Add(SC_DisableRtsCmd_Test_Nominal, SC_Test_Setup, SC_Test_TearDown,
               "SC_DisableRtsCmd_Test_Nominal");
    UtTest_Add(SC_DisableRtsCmd_Test_InvalidRtsID, SC_Test_Setup,
               SC_Test_TearDown, "SC_DisableRtsCmd_Test_InvalidRtsID");
    UtTest_Add(SC_DisableRtsGrpCmd_Test_Nominal, SC_Test_Setup,
               SC_Test_TearDown, "SC_DisableRtsGrpCmd_Test_Nominal");
    UtTest_Add(SC_DisableRtsGrpCmd_Test_Error, SC_Test_Setup, SC_Test_TearDown,
               "SC_DisableRtsGrpCmd_Test_Error");
    UtTest_Add(SC_EnableRtsCmd_Test_Nominal, SC_Test_Setup, SC_Test_TearDown,
               "SC_EnableRtsCmd_Test_Nominal");
    UtTest_Add(SC_EnableRtsCmd_Test_InvalidRtsID, SC_Test_Setup,
               SC_Test_TearDown, "SC_EnableRtsCmd_Test_InvalidRtsID");
    UtTest_Add(SC_EnableRtsGrpCmd_Test_Nominal, SC_Test_Setup, SC_Test_TearDown,
               "SC_EnableRtsGrpCmd_Test_Nominal");
    UtTest_Add(SC_EnableRtsGrpCmd_Test_Error, SC_Test_Setup, SC_Test_TearDown,
               "SC_EnableRtsGrpCmd_Test_Error");
    UtTest_Add(SC_KillRts_Test, SC_Test_Setup, SC_Test_TearDown,
               "SC_KillRts_Test");
