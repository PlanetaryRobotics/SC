...
void SC_GetNextRtsTime_Test_Nominal(void) {
    ...
    /* Verify results */
    UtAssert_True(SC_OperData.RtsCtrlBlckAddr->RtsNumber == 1,
                  "SC_OperData.RtsCtrlBlckAddr->RtsNumber == 1");
    UtAssert_True(SC_AppData.NextCmdTime[1] == SC_MAX_TIME,
                  "SC_AppData.NextCmdTime[1] == SC_MAX_TIME");
...
void SC_GetNextRtsTime_Test_InvalidRtsNumber(void) {
    ...
    UtAssert_True(
        ...
        "SC_OperData.RtsCtrlBlckAddr->RtsNumber == SC_INVALID_RTS_NUMBER");
    UtAssert_True(SC_AppData.NextCmdTime[SC_RTP] == SC_MAX_TIME,
...
void SC_UpdateNextTime_TestAtp(void) {
    ...
    /* Verify results */
    UtAssert_True(SC_AppData.NextProcNumber == SC_ATP,
...
void SC_UpdateNextTime_TestRtp(void) {
    ...
    /* Verify results */
    UtAssert_True(SC_AppData.NextProcNumber == SC_RTP,
...
void SC_GetNextRtsCommand_Test_GetNextCommand(void) {
    ...
    /* Give the packet the minimum possible size, to ensure that (CmdOffset <
     * (SC_RTS_BUFF_SIZE - (sizeof(SC_RtsEntryHeader_t) / SC_BYTES_IN_WORD) is
     * met */
    Ut_CFE_SB_SetReturnCode(UT_CFE_SB_GETTOTALMSGLENGTH_INDEX,
                            SC_PACKET_MIN_SIZE, 1);
    Ut_CFE_SB_ContinueReturnCodeAfterCountZero(
        UT_CFE_SB_GETTOTALMSGLENGTH_INDEX);

    /* Execute the function being tested */
    /* NOTE: Calling SC_ProcessRtpCommand instead of SC_GetNextRtsCommand -
     * SC_ProcessRtpCommand calls SC_GetNextRtsCommand, and it's much to test
     * easier this way. */
    ...
    UtAssert_True(
        ...
        "SC_OperData.RtsInfoTblAddr[0].NextCommandPtr == (SC_PACKET_MIN_SIZE + "
        "SC_RTS_HEADER_SIZE + 1) / SC_BYTES_IN_WORD");
...
void SC_GetNextRtsCommand_Test_RtsLengthError(void) {
    ...
    SC_OperData.RtsInfoTblAddr[0].NextCommandPtr =
        (SC_RTS_BUFF_SIZE - (sizeof(SC_RtsEntryHeader_t) / SC_BYTES_IN_WORD)) -
        ((SC_PACKET_MAX_SIZE + SC_RTS_HEADER_SIZE + 1) / SC_BYTES_IN_WORD) - 1;

    /* Execute the function being tested */
    /* NOTE: Calling SC_ProcessRtpCommand instead of SC_GetNextRtsCommand -
     * SC_ProcessRtpCommand calls SC_GetNextRtsCommand, and it's much easier to
     * test this way. */
    SC_ProcessRtpCommand();
    ...
    UtAssert_True(
        SC_AppData.LastRtsErrCmd ==
            SC_OperData.RtsInfoTblAddr[0].NextCommandPtr +
                ((SC_PACKET_MAX_SIZE + SC_RTS_HEADER_SIZE + 1) /
                 SC_BYTES_IN_WORD),
        "SC_AppData.LastRtsErrCmd == "
        "SC_OperData.RtsInfoTblAddr[0].NextCommandPtr + ((SC_PACKET_MAX_SIZE + "
        "SC_RTS_HEADER_SIZE + 1) / SC_BYTES_IN_WORD)");

    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_RTS_LNGTH_ERR_EID, CFE_EVS_ERROR,
...
void SC_GetNextRtsCommand_Test_CommandLengthError(void) {
    ...
    /* Set to make checksum pass in SC_ProcessRtpCommand, so that
     * SC_GetNextRtsCommand is reached */
    Ut_CFE_SB_SetReturnCode(UT_CFE_SB_VALIDATECHECKSUM_INDEX, TRUE, 1);
    Ut_CFE_SB_SetReturnCode(UT_CFE_SB_SENDMSG_INDEX, CFE_SUCCESS, 1);

    SC_OperData.AtsInfoTblAddr[1].NumberOfCommands = 1;

    /* First call to CFE_SB_GetTotalMsgLength returns value to make subsequent
     * if-statement pass, next call returns value to generate error message
     * SC_RTS_LNGTH_ERR_EID */
    Ut_CFE_SB_SetFunctionHook(UT_CFE_SB_GETTOTALMSGLENGTH_INDEX,
                              &SC_STATE_TEST_CFE_SB_GetTotalMsgLengthHook);
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_RTS_CMD_LNGTH_ERR_EID, CFE_EVS_ERROR,
                             "Invalid Length Field in RTS Command, RTS 001 "
                             "Aborted. Length: 350, Max: 250"),
        "Invalid Length Field in RTS Command, RTS 001 Aborted. Length: 350, "
        "Max: 250");
...
void SC_GetNextRtsCommand_Test_ZeroCommandLength(void) {
    ...
    /* Verify results */
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_RTS_COMPL_INF_EID, CFE_EVS_INFORMATION,
...
void SC_GetNextAtsCommand_Test_GetNextCommand(void) {
    ...
    UtAssert_True(SC_OperData.AtsCtrlBlckAddr->CmdNumber ==
                      SC_AppData.AtsTimeIndexBuffer[0][1],
...
void SC_GetNextAtsCommand_Test_ExecutionCompleted(void) {
    ...
    /* Verify results */
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_ATS_COMPL_INF_EID, CFE_EVS_INFORMATION,
...
void SC_State_Test_AddTestCases(void) {
    UtTest_Add(SC_GetNextRtsTime_Test_Nominal, SC_Test_Setup, SC_Test_TearDown,
               "SC_GetNextRtsTime_Test_Nominal");
    UtTest_Add(SC_GetNextRtsTime_Test_InvalidRtsNumber, SC_Test_Setup,
               SC_Test_TearDown, "SC_GetNextRtsTime_Test_InvalidRtsNumber");
    UtTest_Add(SC_UpdateNextTime_TestAtp, SC_Test_Setup, SC_Test_TearDown,
               "SC_UpdateNextTime_TestAtp");
    UtTest_Add(SC_UpdateNextTime_TestRtp, SC_Test_Setup, SC_Test_TearDown,
               "SC_UpdateNextTime_TestRtp");

    UtTest_Add(SC_GetNextRtsCommand_Test_GetNextCommand, SC_Test_Setup,
               SC_Test_TearDown, "SC_GetNextRtsCommand_Test_GetNextCommand");
    UtTest_Add(SC_GetNextRtsCommand_Test_RtsLengthError, SC_Test_Setup,
               SC_Test_TearDown, "SC_GetNextRtsCommand_Test_RtsLengthError");
    UtTest_Add(SC_GetNextRtsCommand_Test_CommandLengthError, SC_Test_Setup,
               SC_Test_TearDown,
               "SC_GetNextRtsCommand_Test_CommandLengthError");
    UtTest_Add(SC_GetNextRtsCommand_Test_ZeroCommandLength, SC_Test_Setup,
               SC_Test_TearDown, "SC_GetNextRtsCommand_Test_ZeroCommandLength");
    UtTest_Add(SC_GetNextRtsCommand_Test_EndOfBuffer, SC_Test_Setup,
               SC_Test_TearDown, "SC_GetNextRtsCommand_Test_EndOfBuffer");

    UtTest_Add(SC_GetNextAtsCommand_Test_Starting, SC_Test_Setup,
               SC_Test_TearDown, "SC_GetNextAtsCommand_Test_Starting");
    UtTest_Add(SC_GetNextAtsCommand_Test_GetNextCommand, SC_Test_Setup,
               SC_Test_TearDown, "SC_GetNextAtsCommand_Test_GetNextCommand");
    UtTest_Add(SC_GetNextAtsCommand_Test_ExecutionCompleted, SC_Test_Setup,
               SC_Test_TearDown,
               "SC_GetNextAtsCommand_Test_ExecutionCompleted");
