...
/* STRINGIFY(x) is a preprocessor macro to convert preprocessor constants into
 * strings */
#define STR(x)       #x
...
CFE_TIME_Compare_t Ut_CFE_TIME_CompareHookAlessthanB(CFE_TIME_SysTime_t TimeA,
...
void SC_ProcessAtpCmd_Test_SwitchCmd(void) {
    ...
    /* Set return value for CFE_TIME_Compare to make SC_CompareAbsTime return
     * FALSE, to satisfy first if-statement of SC_ProcessAtpCmd, and for all
     * other calls to CFE_TIME_Compare called from subfunctions reached by this
     * test */
    Ut_CFE_TIME_SetFunctionHook(UT_CFE_TIME_COMPARE_INDEX,
                                &Ut_CFE_TIME_CompareHookAlessthanB);

    /* Set to return TRUE in order to satisfy the if-statement from which the
     * function is called */
    Ut_CFE_SB_SetReturnCode(UT_CFE_SB_VALIDATECHECKSUM_INDEX, TRUE, 1);

    /* Set these two functions to return these values in order to statisfy the
     * if-statement from which they are both called */
    ...
    /* Same return value as default, but bypasses default hook function to
     * simplify test */
    Ut_CFE_PSP_MEMUTILS_SetReturnCode(UT_CFE_PSP_MEMUTILS_MEMCPY_INDEX,
                                      CFE_PSP_SUCCESS, 1);
...
void SC_ProcessAtpCmd_Test_NonSwitchCmd(void) {
    ...
    /* Set to return this value in order to statisfy the if-statement from which
     * it is called */
    Ut_CFE_SB_SetReturnCode(UT_CFE_SB_GETMSGID_INDEX, SC_CMD_MID, 1);

    /* Set to return this value in order fail the if-statement from which it is
     * called */
...
void SC_ProcessAtpCmd_Test_InlineSwitchError(void) {
    ...
    UtAssert_True(SC_AppData.LastAtsErrSeq == 1,
                  "SC_AppData.LastAtsErrSeq == 1");
    UtAssert_True(SC_AppData.LastAtsErrCmd == 1,
                  "SC_AppData.LastAtsErrCmd == 1");
...
void SC_ProcessAtpCmd_Test_SBErrorAtsA(void) {
    ...
    UtAssert_True(SC_OperData.AtsCmdStatusTblAddr[0][0] == SC_FAILED_DISTRIB,
                  "SC_OperData.AtsCmdStatusTblAddr[0][0] == SC_FAILED_DISTRIB");
    UtAssert_True(SC_AppData.LastAtsErrSeq == SC_ATSA,
                  "SC_AppData.LastAtsErrSeq == SC_ATSA");
    UtAssert_True(SC_AppData.LastAtsErrCmd == 1,
                  "SC_AppData.LastAtsErrCmd == 1");

    UtAssert_True(Ut_CFE_EVS_EventSent(SC_ATS_DIST_ERR_EID, CFE_EVS_ERROR,
                                       "ATS Command Distribution Failed, Cmd "
                                       "Number: 1, SB returned: 0xFFFFFFFF"),
                  "ATS Command Distribution Failed, Cmd Number: 1, SB "
                  "returned: 0xFFFFFFFF");

    UtAssert_True(Ut_CFE_EVS_EventSent(SC_ATS_ABT_ERR_EID, CFE_EVS_ERROR,
                                       "ATS A Aborted"),
                  "ATS A Aborted");
...
void SC_ProcessAtpCmd_Test_SBErrorAtsB(void) {
    ...
    UtAssert_True(SC_OperData.AtsCmdStatusTblAddr[1][0] == SC_FAILED_DISTRIB,
                  "SC_OperData.AtsCmdStatusTblAddr[1][0] == SC_FAILED_DISTRIB");
    UtAssert_True(SC_AppData.LastAtsErrSeq == SC_ATSB,
                  "SC_AppData.LastAtsErrSeq == SC_ATSB");
    UtAssert_True(SC_AppData.LastAtsErrCmd == 1,
                  "SC_AppData.LastAtsErrCmd == 1");
...
void SC_ProcessAtpCmd_Test_ChecksumFailedAtsA(void) {
    ...
    /* Set to return FALSE in order to generate error message
    ...
    UtAssert_True(SC_AppData.LastAtsErrCmd == 1,
                  "SC_AppData.LastAtsErrCmd == 1");
    UtAssert_True(
        SC_OperData.AtsCmdStatusTblAddr[0][0] == SC_FAILED_CHECKSUM,
        "SC_OperData.AtsCmdStatusTblAddr[1][0] == SC_FAILED_CHECKSUM");

    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_ATS_CHKSUM_ERR_EID, CFE_EVS_ERROR,
...
void SC_ProcessAtpCmd_Test_CmdNumberMismatchAtsA(void) {
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(SC_ATS_MSMTCH_ERR_EID, CFE_EVS_ERROR,
                                       "ATS Command Number Mismatch: Command "
                                       "Skipped, expected: 1 received: 3"),
                  "ATS Command Number Mismatch: Command Skipped, expected: 1 "
                  "received: 3");
...
void SC_ProcessAtpCmd_Test_CmdNotLoaded(void) {
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(
                      SC_ATS_SKP_ERR_EID, CFE_EVS_ERROR,
...
void SC_ProcessRtpCommand_Test_Nominal(void) {
    ...
    SC_OperData.RtsInfoTblAddr[SC_OperData.RtsCtrlBlckAddr->RtsNumber - 1]
        .RtsStatus = SC_EXECUTING;
    ...
    UtAssert_True(SC_OperData.NumCmdsSec == 1, "SC_OperData.NumCmdsSec == 1");
    UtAssert_True(SC_OperData.RtsInfoTblAddr[0].CmdCtr == 1,
                  "SC_OperData.RtsInfoTblAddr[0].CmdCtr == 1");
    UtAssert_True(SC_OperData.RtsInfoTblAddr[0].CmdErrCtr == 0,
                  "SC_OperData.RtsInfoTblAddr[0].CmdErrCtr == 0");
...
void SC_ProcessRtpCommand_Test_BadSoftwareBusReturn(void) {
    ...
    UtAssert_True(SC_OperData.NumCmdsSec == 1, "SC_OperData.NumCmdsSec == 1");
    UtAssert_True(SC_OperData.RtsInfoTblAddr[0].CmdCtr == 0,
                  "SC_OperData.RtsInfoTblAddr[0].CmdCtr == 0");
    UtAssert_True(SC_OperData.RtsInfoTblAddr[0].CmdErrCtr == 1,
                  "SC_OperData.RtsInfoTblAddr[0].CmdErrCtr == 1");
    UtAssert_True(SC_AppData.LastRtsErrSeq == 1,
                  "SC_AppData.LastRtsErrSeq == 1");
    UtAssert_True(SC_AppData.LastRtsErrCmd == 0,
                  "SC_AppData.LastRtsErrCmd == 0");

    UtAssert_True(Ut_CFE_EVS_EventSent(SC_RTS_DIST_ERR_EID, CFE_EVS_ERROR,
                                       "RTS 001 Command Distribution Failed: "
                                       "RTS Stopped. SB returned 0xFFFFFFFF"),
                  "RTS 001 Command Distribution Failed: RTS Stopped. SB "
                  "returned 0xFFFFFFFF");
...
void SC_ProcessRtpCommand_Test_BadChecksum(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_RTS_CHKSUM_ERR_EID, CFE_EVS_ERROR,
...
void SC_SendHkPacket_Test(void) {
    ...
    /* Verify results */
    UtAssert_True(SC_OperData.HkPacket.CmdErrCtr == 1,
                  "SC_OperData.HkPacket.CmdErrCtr == 1");
    UtAssert_True(SC_OperData.HkPacket.CmdCtr == 2,
                  "SC_OperData.HkPacket.CmdCtr == 2");
    UtAssert_True(SC_OperData.HkPacket.RtsActiveErrCtr == 3,
                  "SC_OperData.HkPacket.RtsActiveErrCtr == 3");
    UtAssert_True(SC_OperData.HkPacket.RtsActiveCtr == 4,
                  "SC_OperData.HkPacket.RtsActiveCtr == 4");
    UtAssert_True(SC_OperData.HkPacket.AtsCmdCtr == 5,
                  "SCSC_OperData.HkPacket.AtsCmdCtr == 5");
    UtAssert_True(SC_OperData.HkPacket.AtsCmdErrCtr == 6,
                  "SC_OperData.HkPacket.AtsCmdErrCtr == 6");
    UtAssert_True(SC_OperData.HkPacket.RtsCmdCtr == 7,
                  "SC_OperData.HkPacket.RtsCmdCtr == 7");
    UtAssert_True(SC_OperData.HkPacket.RtsCmdErrCtr == 8,
                  "SC_OperData.HkPacket.RtsCmdErrCtr == 8");
    UtAssert_True(SC_OperData.HkPacket.LastAtsErrSeq == 9,
                  "SC_OperData.HkPacket.LastAtsErrSeq == 9");
    UtAssert_True(SC_OperData.HkPacket.LastAtsErrCmd == 10,
                  "SC_OperData.HkPacket.LastAtsErrCmd == 10");
    UtAssert_True(SC_OperData.HkPacket.LastRtsErrSeq == 11,
                  "SC_OperData.HkPacket.LastRtsErrSeq == 11");
    UtAssert_True(SC_OperData.HkPacket.LastRtsErrCmd == 12,
                  "SC_OperData.HkPacket.LastRtsErrCmd == 12");
    UtAssert_True(SC_OperData.HkPacket.AppendCmdArg == 13,
                  "SC_OperData.HkPacket.AppendCmdArg == 13");
    UtAssert_True(SC_OperData.HkPacket.AppendEntryCount == 14,
                  "SC_OperData.HkPacket.AppendEntryCount == 14");
    UtAssert_True(SC_OperData.HkPacket.AppendByteCount == 30,
                  "SC_OperData.HkPacket.AppendByteCount == 30");
    UtAssert_True(SC_OperData.HkPacket.AppendLoadCount == 16,
                  "SC_OperData.HkPacket.AppendLoadCount == 16");
    UtAssert_True(SC_OperData.HkPacket.AtpFreeBytes[0] ==
                      (SC_ATS_BUFF_SIZE * SC_BYTES_IN_WORD),
                  "SC_OperData.HkPacket.AtpFreeBytes[0] == (SC_ATS_BUFF_SIZE * "
                  "SC_BYTES_IN_WORD)");
    UtAssert_True(SC_OperData.HkPacket.AtpFreeBytes[1] ==
                      (SC_ATS_BUFF_SIZE * SC_BYTES_IN_WORD),
                  "SC_OperData.HkPacket.AtpFreeBytes[1] == (SC_ATS_BUFF_SIZE * "
                  "SC_BYTES_IN_WORD)");
    UtAssert_True(SC_OperData.HkPacket.AtsNumber == 17,
                  "SC_OperData.HkPacket.AtsNumber == 17");
    UtAssert_True(SC_OperData.HkPacket.AtpState == 18,
                  "SC_OperData.HkPacket.AtpState == 18");
    UtAssert_True(SC_OperData.HkPacket.AtpCmdNumber == 20,
                  "SC_OperData.HkPacket.AtpCmdNumber == 20");
    UtAssert_True(SC_OperData.HkPacket.SwitchPendFlag == 0,
                  "SC_OperData.HkPacket.SwitchPendFlag == 0");
    UtAssert_True(SC_OperData.HkPacket.NextAtsTime == 0,
                  "SC_OperData.HkPacket.NextAtsTime == 0");
    UtAssert_True(SC_OperData.HkPacket.NumRtsActive == 20,
                  "SC_OperData.HkPacket.NumRtsActive == 20");
    UtAssert_True(SC_OperData.HkPacket.RtsNumber == 21,
                  "SC_OperData.HkPacket.RtsNumber == 21");
    UtAssert_True(SC_OperData.HkPacket.NextRtsTime == 0,
                  "SC_OperData.HkPacket.NextRtsTime == 0");
    UtAssert_True(SC_OperData.HkPacket.ContinueAtsOnFailureFlag == 1,
                  "SC_OperData.HkPacket.ContinueAtsOnFailureFlag == 1");

    /* Check first element */
    UtAssert_True(SC_OperData.HkPacket.RtsExecutingStatus[0] == 31,
                  "SC_OperData.HkPacket.RtsExecutingStatus[0] == 31");
    UtAssert_True(SC_OperData.HkPacket.RtsDisabledStatus[0] == 31,
                  "SC_OperData.HkPacket.RtsDisabledStatus[0] == 31");

    /* Check middle element.  Note: (SC_NUMBER_OF_RTS+15)/16 is taken directly
     * from the source code */
    UtAssert_True(
        SC_OperData.HkPacket
                .RtsExecutingStatus[((SC_NUMBER_OF_RTS + 15) / 16) / 2] == 31,
        "SC_OperData.HkPacket.RtsExecutingStatus[((SC_NUMBER_OF_RTS+15)/16) / "
        "2] == 31");
    UtAssert_True(
        SC_OperData.HkPacket
                .RtsDisabledStatus[((SC_NUMBER_OF_RTS + 15) / 16) / 2] == 31,
        "SC_OperData.HkPacket.RtsDisabledStatus[((SC_NUMBER_OF_RTS+15)/16) / "
        "2] == 31");

    /* Check last element.  Note: (SC_NUMBER_OF_RTS+15)/16 is taken directly
     * from the source code */
    UtAssert_True(
        SC_OperData.HkPacket
                .RtsExecutingStatus[((SC_NUMBER_OF_RTS + 15) / 16) - 1] == 31,
        "SC_OperData.HkPacket.RtsExecutingStatus[((SC_NUMBER_OF_RTS+15)/16) - "
        "1] == 31");
    UtAssert_True(
        SC_OperData.HkPacket
                .RtsDisabledStatus[((SC_NUMBER_OF_RTS + 15) / 16) - 1] == 31,
        "SC_OperData.HkPacket.RtsDisabledStatus[((SC_NUMBER_OF_RTS+15)/16) - "
        "1] == 31");
...
void SC_ProcessRequest_Test_CmdMID(void) {
    ...
    sprintf(Message, "No-op command. Version %d.%d.%d.%d", SC_MAJOR_VERSION,
            SC_MINOR_VERSION, SC_REVISION, SC_MISSION_REV);
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_NOOP_INF_EID, CFE_EVS_INFORMATION, Message),
        Message);
...
void SC_ProcessRequest_Test_1HzWakeupAtpNotExecutionTime(void) {
    /**
     **  Test case: SC_1HZ_WAKEUP_MID with a pending ATP command that should not
     *execute yet
     **/
...
void SC_ProcessRequest_Test_1HzWakeupRtpExecutionTime(void) {
    /**
     **  Test case: SC_1HZ_WAKEUP_MID with a pending RTP command that needs to
     *execute immediately
     **/
    ...
    SC_AppData.NextProcNumber = SC_RTP;
    SC_OperData.AtsCtrlBlckAddr->AtpState =
        SC_EXECUTING; /* Causes switch to ATP */
    SC_AppData.NextCmdTime[SC_ATP] = 0;
...
void SC_ProcessRequest_Test_1HzWakeupRtpExecutionTimeTooManyCmds(void) {
    /**
     **  Test case: SC_1HZ_WAKEUP_MID with a pending RTP command that needs to
     *execute immediately, but too many commands are being sent at once
     **/
...
void SC_ProcessRequest_Test_MIDError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_MID_ERR_EID, CFE_EVS_ERROR,
...
void SC_ProcessCommand_Test_NoOp(void) {
    /**
     **  Note: This test does not follow the standard test guideline to only
     *test what's directly in the function-under-test.
     **  Since the code for reaching each branch in SC_ProcessCommand is so
     *trivial and non-verifiable, it was decided to
     **  combine the tests for each command with the tests for reaching the
     *command from SC_ProcessCommand.
     **/
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            SC_NOOP_INF_EID, CFE_EVS_INFORMATION,
            "No-op command. Version " STRINGIFY(SC_MAJOR_VERSION) "." STRINGIFY(
                SC_MINOR_VERSION) "." STRINGIFY(SC_REVISION) "." STRINGIFY(SC_MISSION_REV)),
        "No-op command. Version " STRINGIFY(SC_MAJOR_VERSION) "." STRINGIFY(
            SC_MINOR_VERSION) "." STRINGIFY(SC_REVISION) "." STRINGIFY(SC_MISSION_REV));
...
void SC_ProcessCommand_Test_ResetCounters(void) {
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(SC_RESET_DEB_EID, CFE_EVS_DEBUG,
...
void SC_ProcessCommand_Test_StartAts(void) {
    ...
    /* This function is already verified to work correctly in another file, so
...
void SC_ProcessCommand_Test_TableManageAtsTableGetAddressError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            SC_TABLE_MANAGE_ATS_ERR_EID, CFE_EVS_ERROR,
...
void SC_ProcessCommand_Test_TableManageAppendTableGetAddressError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            SC_TABLE_MANAGE_APPEND_ERR_EID, CFE_EVS_ERROR,
...
void SC_ProcessCommand_Test_TableManageRtsTableGetAddressError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            SC_TABLE_MANAGE_RTS_ERR_EID, CFE_EVS_ERROR,
...
void SC_ProcessCommand_Test_TableManageInvalidTableID(void) {
    ...
    /* Verify results */
    UtAssert_True(Ut_CFE_EVS_EventSent(
                      SC_TABLE_MANAGE_ID_ERR_EID, CFE_EVS_ERROR,
...
void SC_ProcessCommand_Test_InvalidCmdError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(SC_INVLD_CMD_ERR_EID, CFE_EVS_ERROR,
...
/* SC_VerifyCmdLength_Test_Nominal not needed - nominal case already tested in
 * other functions. */
...
void SC_Cmds_Test_AddTestCases(void) {
    UtTest_Add(SC_ProcessAtpCmd_Test_SwitchCmd, SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessAtpCmd_Test_SwitchCmd");
    UtTest_Add(SC_ProcessAtpCmd_Test_NonSwitchCmd, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessAtpCmd_Test_NonSwitchCmd");

    UtTest_Add(SC_ProcessAtpCmd_Test_InlineSwitchError, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessAtpCmd_Test_InlineSwitchError");
    UtTest_Add(SC_ProcessAtpCmd_Test_SBErrorAtsA, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessAtpCmd_Test_SBErrorAtsA");
    UtTest_Add(SC_ProcessAtpCmd_Test_SBErrorAtsB, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessAtpCmd_Test_SBErrorAtsB");
    UtTest_Add(SC_ProcessAtpCmd_Test_ChecksumFailedAtsA, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessAtpCmd_Test_ChecksumFailedAtsA");
    UtTest_Add(SC_ProcessAtpCmd_Test_ChecksumFailedAtsB, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessAtpCmd_Test_ChecksumFailedAtsB");
    UtTest_Add(SC_ProcessAtpCmd_Test_CmdNumberMismatchAtsA, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessAtpCmd_Test_CmdNumberMismatchAtsA");
    UtTest_Add(SC_ProcessAtpCmd_Test_CmdNumberMismatchAtsB, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessAtpCmd_Test_CmdNumberMismatchAtsB");
    UtTest_Add(SC_ProcessAtpCmd_Test_CmdNotLoaded, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessAtpCmd_Test_CmdNotLoaded");

    UtTest_Add(SC_ProcessRtpCommand_Test_Nominal, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessRtpCommand_Test_Nominal");
    UtTest_Add(SC_ProcessRtpCommand_Test_BadSoftwareBusReturn, SC_Test_Setup,
               SC_Test_TearDown,
               "SC_ProcessRtpCommand_Test_BadSoftwareBusReturn");
    UtTest_Add(SC_ProcessRtpCommand_Test_BadChecksum, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessRtpCommand_Test_BadChecksum");

    UtTest_Add(SC_SendHkPacket_Test, SC_Test_Setup, SC_Test_TearDown,
               "SC_SendHkPacket_Test");
    UtTest_Add(SC_ProcessRequest_Test_CmdMID, SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessRequest_Test_CmdMID");
    UtTest_Add(SC_ProcessRequest_Test_HkMID, SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessRequest_Test_HkMID");
    UtTest_Add(SC_ProcessRequest_Test_1HzWakeupNONE, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessRequest_Test_1HzWakeupNONE");
    UtTest_Add(SC_ProcessRequest_Test_1HzWakeupAtpNotExecutionTime,
               SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessRequest_Test_1HzWakeupAtpNotExecutionTime");
    UtTest_Add(SC_ProcessRequest_Test_1HzWakeupRtpExecutionTime, SC_Test_Setup,
               SC_Test_TearDown,
               "SC_ProcessRequest_Test_1HzWakeupRtpExecutionTime");
    UtTest_Add(SC_ProcessRequest_Test_1HzWakeupRtpExecutionTimeTooManyCmds,
               SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessRequest_Test_1HzWakeupRtpExecutionTimeTooManyCmds");
    UtTest_Add(SC_ProcessRequest_Test_MIDError, SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessRequest_Test_MIDError");
    UtTest_Add(SC_ProcessCommand_Test_NoOp, SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessCommand_Test_NoOp");
    UtTest_Add(SC_ProcessCommand_Test_ResetCounters, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_ResetCounters");
    UtTest_Add(SC_ProcessCommand_Test_StartAts, SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessCommand_Test_StartAts");
    UtTest_Add(SC_ProcessCommand_Test_StopAts, SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessCommand_Test_StopAts");

    UtTest_Add(SC_ProcessCommand_Test_StartRts, SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessCommand_Test_StartRts");
    UtTest_Add(SC_ProcessCommand_Test_StopRts, SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessCommand_Test_StopRts");
    UtTest_Add(SC_ProcessCommand_Test_DisableRts, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_DisableRts");
    UtTest_Add(SC_ProcessCommand_Test_EnableRts, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_EnableRts");
    UtTest_Add(SC_ProcessCommand_Test_SwitchAts, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_SwitchAts");
    UtTest_Add(SC_ProcessCommand_Test_JumpAts, SC_Test_Setup, SC_Test_TearDown,
               "SC_ProcessCommand_Test_JumpAts");
    UtTest_Add(SC_ProcessCommand_Test_ContinueAtsOnFailure, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_ContinueAtsOnFailure");
    UtTest_Add(SC_ProcessCommand_Test_AppendAts, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_AppendAts");

    UtTest_Add(SC_ProcessCommand_Test_TableManageAtsTableNominal, SC_Test_Setup,
               SC_Test_TearDown,
    ...
    UtTest_Add(SC_ProcessCommand_Test_TableManageRtsTableNominal, SC_Test_Setup,
               SC_Test_TearDown,
    ...
    UtTest_Add(SC_ProcessCommand_Test_TableManageRtsInfo, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_TableManageRtsInfo");
    UtTest_Add(SC_ProcessCommand_Test_TableManageRtpCtrl, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_TableManageRtpCtrl");
    UtTest_Add(SC_ProcessCommand_Test_TableManageAtsInfo, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_TableManageAtsInfo");
    UtTest_Add(SC_ProcessCommand_Test_TableManageAppendInfo, SC_Test_Setup,
               SC_Test_TearDown,
               "SC_ProcessCommand_Test_TableManageAppendInfo");
    UtTest_Add(SC_ProcessCommand_Test_TableManageAtpCtrl, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_TableManageAtpCtrl");
    UtTest_Add(SC_ProcessCommand_Test_TableManageAtsCmdStatus, SC_Test_Setup,
               SC_Test_TearDown,
               "SC_ProcessCommand_Test_TableManageAtsCmdStatus");
    UtTest_Add(SC_ProcessCommand_Test_TableManageInvalidTableID, SC_Test_Setup,
               SC_Test_TearDown,
               "SC_ProcessCommand_Test_TableManageInvalidTableID");

    UtTest_Add(SC_ProcessCommand_Test_StartRtsGrp, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_StartRtsGrp");
    UtTest_Add(SC_ProcessCommand_Test_StopRtsGrp, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_StopRtsGrp");
    UtTest_Add(SC_ProcessCommand_Test_DisableRtsGrp, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_DisableRtsGrp");
    UtTest_Add(SC_ProcessCommand_Test_EnableRtsGrp, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_EnableRtsGrp");
    UtTest_Add(SC_ProcessCommand_Test_InvalidCmdError, SC_Test_Setup,
               SC_Test_TearDown, "SC_ProcessCommand_Test_InvalidCmdError");
    UtTest_Add(SC_VerifyCmdLength_Test_LenError, SC_Test_Setup,
